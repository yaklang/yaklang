...
Usage:
  python3 ssa-tree.py [--data-dir ./ci-ssa-data] [--worktree <repo-root>]
"""
import argparse
import json
import os
...
DEFAULT_WORKTREE = str(Path(__file__).resolve().parents[1])
...
def find_clone_dirs() -> list[Path]:
    """Find ci-promote-work-* directories in temp dir."""
    import glob
    import tempfile
    pattern = str(Path(tempfile.gettempdir()) / "ci-promote-work-*")
    return [Path(d) for d in sorted(glob.glob(pattern)) if Path(d).is_dir()]
...
def classify_program(name: str, manifest_base: str, pointer_base: str) -> str:
    ...
    if name.startswith("ci-yaklang-promote-") or name.startswith("ci-yaklang-pr-"):
...
def build_promote_chain(programs: list[str]) -> list[str]:
    ...
    promote_progs = sorted(
        [p for p in programs if p.startswith("ci-yaklang-promote-") or p.startswith("ci-yaklang-pr-")],
...
def main():
    ...
    for clone_dir in clone_dirs:
        ...
        if clone_data.is_symlink():
            # It's a symlink — either to the main data dir (promote clone) or
            # elsewhere. Either way there's no independent DB to show; skip.
            continue
