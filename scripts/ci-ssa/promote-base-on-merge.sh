...
# ---------------------------------------------------------------------------
SSA_CI_DATA_DIR="${SSA_CI_DATA_DIR:-./ci-ssa-data}"
...
promote_once() {
  ...
  # Name the promote overlay after the PR number (ci-yaklang-pr-<N>) when
  # available; fall back to sha-suffix naming only without a PR number.
  local NEW_PROG
  if [ -n "$PR_NUM" ]; then
    NEW_PROG="ci-yaklang-pr-${PR_NUM}"
  else
