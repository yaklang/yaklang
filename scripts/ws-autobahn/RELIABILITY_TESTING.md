...
- RFC 6455 握手：`Upgrade`、`Connection`、`Sec-WebSocket-Accept`、子协议、信息响应链、
  无 body 状态和 HTTP/WebSocket 字节边界。
...
Autobahn fuzzing server 使用规范且基本固定的 `101` 响应，主要验证连接建立后的帧、
关闭和压缩行为。它不覆盖 `101 + Transfer-Encoding`、`101 + Content-Length`、多个
`1xx`、代理追加异常头或首帧与响应头的 TCP 分包组合；这些场景必须由 Yak 自建的
opening-handshake matrix 单独验证。
...
go test ./common/utils \
  -run 'TestReadSingleHTTPResponseNoBodyStatusPreservesFollowingBytes|TestReadHTTPResponseSkipsInformationalChainAndPreservesFollowingBytes' \
  -count=1

go test ./common/utils/lowhttp \
  -run 'TestWebsocket|TestStrictWebsocket|TestCompressedFragments|TestWebsocketClientOpeningHandshakeBoundaryMatrix' \
  -count=1
...
HTTP 状态码语义必须先于 `Content-Length` 和 `Transfer-Encoding` 决定是否读取 body。
`1xx`、`204` 和 `304` 没有消息体；尤其是 `101` 头结束后连接已经切换协议，即使上游
错误地携带 `Transfer-Encoding: chunked`，也不能启动 chunked decoder。WebSocket
客户端使用 `bufio.Reader(conn)` 保存预读帧，并在其上放置 `TeeReader`，确保响应 hook
只记录解析器实际消费的最终 `101`。回归矩阵覆盖：

- `101 + Transfer-Encoding: chunked + 首帧同包`。
- `101 + Content-Length + 首帧同包`。
- 代理追加头、首帧分包和半帧预读。
- `100/102/103 -> 101` 信息响应链。
- `204/304 + Transfer-Encoding` 不消费后续连接字节。
- 非法 `Upgrade` / `Connection` 拒绝及无害重复 token 兼容。
- 浏览器经 Yak MITM 收到服务端首帧并完成双向消息转发。
...
当前进度：P0 的本地 full core/compression、Vulinbox、opening-handshake matrix 和
WebSocket 专项 race 已完成；nightly CI 接入尚未完成。下一主线应进入 P1，并优先处理已被完整压缩测试
...
- `lowhttp` 完整测试通过。
- HTTP 无 body 状态和 WebSocket opening-handshake matrix 通过。
- `101 + Transfer-Encoding: chunked + 服务端首帧` 的真实 MITM 双向测试通过。
