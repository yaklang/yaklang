...
go test -timeout 20s ./common/utils/dbcache/...
go test -timeout 20s ./common/yak/ssa/...
go test -timeout 1m ./common/yak/ssa_compile
...
# Measured baseline on this repo: ~62-64s for the full package (direct
# `go test -count=1 -timeout 5m` runs: 61.6s/62.5s/64.3s). Keep a 3x margin
# so slow CI hosts do not trip the package timeout.
go test -v -timeout 3m ./common/yak/syntaxflow_scan/...
