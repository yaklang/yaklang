...
echo "SSA URL"
go test -count=1 -timeout 1m -run '^(TestSFURL|TestSFURL_golang)$' github.com/yaklang/yaklang/common/yak/yakurl
...
go test -count=1 -timeout 5m -run TestGRPCMUSTPASS_QuerySyntaxFlowScanTask* ./common/yakgrpc/...
