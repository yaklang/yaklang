...
}	}// GetEndpointManager returns the endpoint manager for creating review endpoints
func (c *Config) GetEndpointManager() *endpointManager {
	if c.endpointManager == nil {
		c.endpointManager = newEndpointManagerContext(c.ctx)
		c.endpointManager.config = c
	}
	return c.endpointManager
}

// NewEndpointForParameterReview creates an endpoint specifically for parameter review
func (c *Config) NewEndpointForParameterReview() *Endpoint {
	return c.GetEndpointManager().createEndpoint()
}

// NewEndpointForToolReview creates an endpoint specifically for tool review  
func (c *Config) NewEndpointForToolReview() *Endpoint {
	return c.GetEndpointManager().createEndpoint()
}

// DoWaitAgree waits for user agreement on the endpoint
func (c *Config) DoWaitAgree(ep *Endpoint) {
	ep.Wait()
}

// EmitRequireReviewForToolUse emits a tool use review event
func (c *Config) EmitRequireReviewForToolUse(tool *aitool.Tool, params aitool.InvokeParams, id string) {
	// Implementation for emitting tool use review events
	// This would typically send an event to the client for user review
}

// EmitRequireReviewForParameters emits a parameter review event
func (c *Config) EmitRequireReviewForParameters(tool *aitool.Tool, params aitool.InvokeParams, id string) {
	// Implementation for emitting parameter review events
	// This would typically send an event to the client for user review
}

// ReleaseInteractiveEvent releases an interactive event
func (c *Config) ReleaseInteractiveEvent(id string, params aitool.InvokeParams) {
	c.GetEndpointManager().feed(id, params)
}
