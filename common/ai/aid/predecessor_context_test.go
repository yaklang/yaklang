...
func TestCurrentTaskInfo_PeTaskPrompt_EndToEnd_WithEvidence(t *testing.T) {
	...
	stable := mem.CurrentTaskInfoStable()
	...
	require.Contains(t, stable, peTaskMarkerPrecondition)
	require.NotContains(t, stable, peTaskMarkerSharedEvidence)
	require.NotContains(t, stable, "上面的共享执行证据")
