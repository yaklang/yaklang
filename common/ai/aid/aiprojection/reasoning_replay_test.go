...
func TestExpandReasoningReplayMessagesMalformedAndTerminalFailClosed(t *testing.T) {
	malformed := &aispec.ChatBaseHijackResult{
		IsHijacked: true,
	...
	terminal := &aispec.ChatBaseHijackResult{
		IsHijacked: true,
	...
	emptyAction := &aispec.ChatBaseHijackResult{
		IsHijacked: true,
	...
	unclosed := &aispec.ChatBaseHijackResult{
		IsHijacked: true,
		Messages: []aispec.ChatDetail{aispec.NewUserChatDetail(
...
func TestExpandReasoningReplayMessagesAllowsMarkerLiteralInEncodedFields(t *testing.T) {
	...
	input := &aispec.ChatBaseHijackResult{
		IsHijacked: true,
		Messages: []aispec.ChatDetail{aispec.NewUserChatDetail(
			wrapReasoningReplayTimeline("before\n" + outer + "\nafter"),
		)},
