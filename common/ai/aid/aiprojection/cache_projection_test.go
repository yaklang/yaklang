...
func projectPrompt(prompt string) *aispec.ChatBaseHijackResult {
	return projectCache(Parse(prompt).Sections())
}

// TestMain 关闭 P2.1 阈值合并 (minCachableUserSegmentBytes = 0), 让本文件
// 既有 happy-path 测试 (4/3 段断言) 都使用很短的 fixture 内容也能命中 happy
// path. 新增的 P2.1 降级路径用例必须用 setProjectionThresholdMerge(t, 1024)
// 显式打开阈值合并, 才能验证降级行为.
//
...
//
// 关键词: aicache, P2.1, projection test main, 阈值合并默认关闭, 降级路径显式开启
...
// extractTextContent 把 projection 输出的 message.Content 提取成 string,
// 兼容两种形态:
//   - string (2 段退化路径 / 3 段路径下的 user2)
//   - []*aispec.ChatContent (3 段路径下的 system / user1, 带 cache_control)
//
// §7.7.7 后 projection 在 3 段路径下会把 system+user1 包成 ChatContent 数组
// 加 ephemeral cache_control 标记, 测试断言 user 内容时需要先用此 helper
// 提取出底层文本再做 contains/equal 判断。
//
// 关键词: aicache, projection test helper, extractTextContent, content 形态兼容
...
// 用于验证 3 段路径下 projection 主动打的 cc 字段。
//
// 关键词: aicache, projection test helper, ephemeral cc 断言
...
// 用于验证 user2 (open 段) 不被 projection 打 cc。
//
// 关键词: aicache, projection test helper, 无 cc 断言
...
// disableProjectionThresholdMerge 临时把 P2.1 阈值合并阈值
...
//	}
//
// 关键词: aicache, projection test helper, P2.1 阈值合并 disable, 测试覆盖
func disableProjectionThresholdMerge(t *testing.T) {
	t.Helper()
	saved := minCachableUserSegmentBytes
	minCachableUserSegmentBytes = 0
...
// setProjectionThresholdMerge 临时把 P2.1 阈值合并阈值设为指定值, 用于 P2.1 降级
// 用例中精细控制阈值 (例如设 50 验证 user1=10 byte 时的合并行为). t.Cleanup
// 自动恢复.
//
// 关键词: aicache, projection test helper, P2.1 阈值合并 setter, 测试覆盖
func setProjectionThresholdMerge(t *testing.T, threshold int) {
	t.Helper()
	saved := minCachableUserSegmentBytes
...
// 关键词: aicache, projection, 仅 high-static 透传
...
// 关键词: aicache, projection, 无 high-static 透传
...
// 关键词: aicache, projection, 多 high-static 拼接
...
func TestHijack_PrefixStable(t *testing.T) {
	...
	r1 := projectPrompt(prompt1)
	r2 := projectPrompt(prompt2)
	require.NotNil(t, r1)
	require.NotNil(t, r2)
...
// 关键词: aicache, projection, AI_CACHE_SYSTEM, 新标签
...
// TestHijack_FixtureFourSection 用真实生产 dump 验证 projection 行为
// 兼容 2 段（timeline 不可拆 frozen/open）与 3 段（timeline 含多 interval 或
// reducer+interval）两种结果。
// 关键词: aicache, projection, fixture 真实数据, 2 段 3 段兼容
func TestHijack_FixtureFourSection(t *testing.T) {
	...
	for _, name := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			// 3 段路径下 system 必须自带 ephemeral cc (§7.7.7 projection 自管 cc)
...
// 关键词: aicache, projection, raw fixture 透传
...
// 关键词: aicache, projection, 3 段 fixture
...
// 的字符串。
//
// 关键词: aicache, projection, 测试构造, timeline 内嵌
...
// 关键词: aicache, projection, 3 段拆分, 多 interval
...
// 关键词: aicache, projection, 3 段拆分, reducer 与 interval 混合
...
// 关键词: aicache, projection, 3 段拆分, 单 interval 加 reducer
...
// 关键词: aicache, projection, 退化 2 段, 单 interval
...
// → 没有 open 段, 应退化到 2 段
// 关键词: aicache, projection, 退化 2 段, 全 reducer
...
// 关键词: aicache, projection, 退化 2 段, 无 timeline
...
// TestHijack_3SegSplit_SystemAndUser1HaveCacheControl 验证 §7.7.7 projection
...
// 关键词: aicache, projection, 双 cc 自管, ephemeral cache_control, §7.7.7
func TestHijack_3SegSplit_SystemAndUser1HaveCacheControl(t *testing.T) {
	...
	require.NotNil(t, res)
	require.True(t, res.IsHijacked)
	require.Len(t, res.Messages, 3, "projection self-managed dual cc requires 3-segment path")
	...
	require.Len(t, sysContents, 1, "projection wraps system into single-element ChatContent slice")
...
// TestHijack_3SegSplit_2SegFallbackHasNoCC 退化到 2 段时, projection 不打 cc
// (system + user 都是 string), 由 aibalance 走"baseline 单 cc 兜底"路径
// 给最末 system 注入 cc。这保证退化路径不破坏 aibalance 现有行为。
// 关键词: aicache, projection, 2 段退化路径, 不打 cc, aibalance 兜底
...
// 关键词: aicache, projection, 字节稳定, 3 段拆分前缀稳定
...
// 验证 projection 优先用 <|AI_CACHE_FROZEN_semi-dynamic|>...
...
//
// 关键词: projection, frozen boundary, 用户案例, §7.7.8 主路径
func TestHijack_FrozenBoundary_UserExampleCase(t *testing.T) {
	...
	// (user1 包含 END 标签是字节边界稳定性的关键, 见 parseCacheBoundaries)
	...
	// system + user1 必须自带 ephemeral cc (§7.7.7 projection 自管双 cc)
...
// 不需要走 timeline 渲染。
// 关键词: projection, frozen boundary, 无 timeline section, 通用切割锚点
...
// 关键词: projection, frozen boundary, 前缀字节稳定
func TestHijack_FrozenBoundary_PrefixStableAcrossOpenChange(t *testing.T) {
	...
	r1 := projectPrompt(mk("open-r1-payload"))
	r2 := projectPrompt(mk("open-r2-completely-different"))
	require.NotNil(t, r1)
	require.NotNil(t, r2)
	require.Len(t, r1.Messages, 3)
	require.Len(t, r2.Messages, 3)
...
// 残缺边界 -> projection 应当退化到 timeline 内部解析路径, 不在残缺位置乱切。
// 关键词: projection, frozen boundary, 残缺边界退化
...
// (病态顺序) -> projection 退化到 timeline 内部解析。
// 关键词: projection, frozen boundary, START 在 END 之后退化
func TestHijack_FrozenBoundary_EndedBeforeStart(t *testing.T) {
	// 注意: boundaryEnd 用 strings.Index 找第一个 START, 然后从 START 后
...
// Timeline 时间戳), 验证 projection 可以正确切割。
// 关键词: projection, frozen boundary, Timeline.Dump 输出格式兼容
...
// projection 切成 4 段消息: [system+cc, user1+cc, user2+cc, user3].
//
// 端到端 prompt 形态:
//
...
// 边界, 没有 semi 边界时, projection 退化到 3 段 (与场景 A 等价).
...
// 出现在 frozen END 之前. parseCacheBoundaries 从 frozenEnd 之后找 semi START,
// 找不到就退化, projection 仍能切 3 段.
...
// 在 frozen 边界内 (frozen END 在 semi END 之后). parseCacheBoundaries 从
...
func TestHijack_SemiBoundary_PrefixStableAcrossDynamicChange(t *testing.T) {
	...
	r1 := projectPrompt(mk("dynamic-r1-payload"))
	r2 := projectPrompt(mk("dynamic-r2-completely-different"))
	require.NotNil(t, r1)
	require.NotNil(t, r2)
...
//   - 阈值边界 (user1 == 阈值) 不触发合并
// 这些用例显式 setProjectionThresholdMerge(t, 1024) 打开默认阈值, 与 TestMain
...
// projection 把 user1 合并到 user2, 退化为 3 段消息: [sys+cc, u12+cc, u3].
...
// 3 段路径降级: 仅有 frozen 边界 (无 semi 边界), user1 < 1KB 时 projection
...
// user2 都 >= 1KB 时, P2.1 阈值合并不应触发, projection 走原 4 段 happy path.
...
func TestHijack_P2_FourSegment_BoundaryEdge_AtThreshold(t *testing.T) {
	...
	setProjectionThresholdMerge(t, 200)
	...
	// 还要算上 parseCacheBoundaries TrimSpace 后剩余的字节. 这里直接用大于
