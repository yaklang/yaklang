...
func TestExtractTaskFromRawResponse_OldFormat_NoIdentifierNoDependsOn(t *testing.T) {
	...
	assert.Nil(t, sub1.DependsOn, "missing depends_on should remain unset")
...
func TestExtractPlan_OldFormat_BackwardCompat(t *testing.T) {
	...
	assert.Nil(t, planResp.RootTask.Subtasks[0].DependsOn)
	assert.Nil(t, planResp.RootTask.Subtasks[1].DependsOn)
}
