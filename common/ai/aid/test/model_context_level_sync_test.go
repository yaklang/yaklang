...
func TestAIDToAIReact_ModelContextLevel_Compact(t *testing.T) {
	...
	if strings.Contains(capturedPrompt, "你处于精简上下文模式下的 ReAct Agent") {
		t.Fatalf("compact context should not switch to a dedicated compact instruction, got: %s", capturedPrompt)
	}
	if !strings.Contains(capturedPrompt, "## 核心行动准则") {
		t.Fatalf("compact context should keep the standard instruction block, got: %s", capturedPrompt)
	}
	if !strings.Contains(capturedPrompt, "## 工具使用指南") {
		t.Fatalf("compact context should keep the standard tool guide block, got: %s", capturedPrompt)
	}
	if !strings.Contains(capturedPrompt, "## Core Traits (核心性格)") {
		t.Fatalf("compact context should keep the standard base traits block, got: %s", capturedPrompt)
	}
}
