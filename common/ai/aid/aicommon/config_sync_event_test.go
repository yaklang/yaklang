...
func TestHandleSyncUpdataConfigEvent_DisablePlanViaSyncJSON(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	syncID := uuid.NewString()
	var syncResponse *schema.AiOutputEvent
	c := NewTestConfig(ctx,
		WithEnablePlanAndExec(true),
	...
	payload := decodeEventPayload(t, syncResponse)
...
func TestHandleSyncUpdataConfigEvent_DisableIntentRecognitionViaSyncJSON(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	c := NewTestConfig(ctx)
	require.False(t, c.GetConfigBool("DisableIntentRecognition"))
	...
	require.NoError(t, err)
	require.True(t, c.GetConfigBool("DisableIntentRecognition"))
}
