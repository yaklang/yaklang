...
| Block 类型                | 来源                                 | 是否冻结        |
| ------------------------- | ------------------------------------ | --------------- |
| `TimelineIntervalBlock`   | 活跃条目先按 N 分钟绝对时间桶分组，再在同一日历桶内按字节预算（默认 16KB）切「子桶」 | 整条 timeline 仅最末 interval 子桶 Open，其余 Frozen |
...
- **字节子桶**：`Timeline.SetTimelineBucketByteSize` / `GroupByMinutesAndBytes` 控制；默认 `TimelineDumpDefaultBucketByteSize`（16KB）。设为负数可关闭字节切分（仅时间桶，nonce 与旧版完全一致）。单条 item 超过预算时独占一个子桶，不在 entry 中间切开。
...
// 等价于 GroupByMinutesAndBytes(minutes, getEffectiveBucketByteSize())，默认叠 16KB 子桶预算。

func (m *Timeline) GroupByMinutesAndBytes(minutes int, bytesPerBucket int64) *TimelineGroups
// bytesPerBucket < 0：关闭字节子桶；== 0：使用 TimelineDumpDefaultBucketByteSize；> 0：使用该字节上限。
