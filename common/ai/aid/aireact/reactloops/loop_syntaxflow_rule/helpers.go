...
const maxSyntaxFlowOutputSummaryBytes = 5 * 1024

func spillSyntaxFlowContent(loop *reactloops.ReActLoop, prefix string, content string) (summary string, reference string) {
	if content == "" {
		return "", ""
	}
	if len(content) <= maxSyntaxFlowOutputSummaryBytes {
	...
	if err := reactloops.SaveAndPinFile(loop, filename, []byte(content)); err != nil {
		log.Warnf("[loop_syntaxflow_rule] failed to save spill content %s: %v", prefix, err)
