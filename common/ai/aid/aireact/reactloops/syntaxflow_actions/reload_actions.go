...
func WithReloadSSARiskOverviewAction(r aicommon.AIInvokeRuntime) reactloops.ReActLoopOption {
	return reactloops.WithRegisterLoopAction(
		...
		func(loop *reactloops.ReActLoop, action *aicommon.Action, operator *reactloops.LoopActionHandlerOperator) {
			...
			db := sfu.GetSSADB()
			filter := sfs.MergeReloadSSARiskOverviewFilter(loop, task, action)
			limit := int64(action.GetInt("limit"))
			summary := sfs.ApplySSARiskOverviewDB(loop, r, db, task, filter, limit)
...
func WithReloadSyntaxFlowScanSessionAction(r aicommon.AIInvokeRuntime) reactloops.ReActLoopOption {
	return reactloops.WithRegisterLoopAction(
		...
		func(loop *reactloops.ReActLoop, action *aicommon.Action, operator *reactloops.LoopActionHandlerOperator) {
			...
			res, err := sfs.LoadScanSessionResult(db, taskID, sfs.DefaultRiskSampleLimit)
			if err != nil {
			...
			sfu.AppendSfScanInterpretLog(loop, r, taskID, "reload_syntaxflow_scan_session: 已刷新任务与 risk 样本")
