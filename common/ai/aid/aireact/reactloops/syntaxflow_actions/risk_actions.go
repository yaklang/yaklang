...
func WithReloadSSARiskAction(r aicommon.AIInvokeRuntime) reactloops.ReActLoopOption {
	return reactloops.WithRegisterLoopAction(
		...
		func(loop *reactloops.ReActLoop, action *aicommon.Action, operator *reactloops.LoopActionHandlerOperator) {
			...
			disposals, _ := sfs.ListRiskDisposals(db, id, true)
			codeLim := 4000
			if action.GetInt("get_full_code") != 0 {
				codeLim = 12000
			}
			text := sfs.RiskReloadText(risk, disposals, codeLim)
...
func WithMarkSSARiskDisposalAction(r aicommon.AIInvokeRuntime) reactloops.ReActLoopOption {
	return reactloops.WithRegisterLoopAction(
		...
		func(loop *reactloops.ReActLoop, action *aicommon.Action, operator *reactloops.LoopActionHandlerOperator) {
			...
			st := sfs.NormalizeDisposalStatus(action.GetString("status"))
...
// WithDeriveRuleSeedFromRiskAction registers derive_rule_seed_from_risk — JSON seed for rule writer / campaign.
func WithDeriveRuleSeedFromRiskAction(_ aicommon.AIInvokeRuntime) reactloops.ReActLoopOption {
	return reactloops.WithRegisterLoopAction(
		"derive_rule_seed_from_risk",
		"Build a structured JSON seed from the given risk_id for write_syntaxflow_rule or batch campaign handoff.",
		[]aitool.ToolOption{
		...
		func(loop *reactloops.ReActLoop, action *aicommon.Action, operator *reactloops.LoopActionHandlerOperator) {
			...
			seed := sfs.RiskToRuleSeedJSON(risk)
			if loop != nil {
