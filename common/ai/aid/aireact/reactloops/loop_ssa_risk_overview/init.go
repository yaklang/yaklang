	...
	err := reactloops.RegisterLoopFactory(
		...
		func(r aicommon.AIInvokeRuntime, opts ...reactloops.ReActLoopOption) (*reactloops.ReActLoop, error) {
			preset := []reactloops.ReActLoopOption{
				...
				reactloops.WithOverrideLoopAction(loopActionDirectlyAnswerSSARiskOverview),
				WithQuerySSARiskOverviewAction(r),
				outputOverviewFindingsAction(r),
				WithAnalyzeFilteredRisksAction(r),
				buildMidIterationGuardHook(r),
				...
				reactloops.WithReactiveDataBuilder(func(loop *reactloops.ReActLoop, feedbacker *bytes.Buffer, nonce string) (string, error) {
					...
					return utils.RenderTemplate(reactiveData, map[string]any{
						...
						"FilterSummary":         loop.Get("ssa_overview_last_filter_summary"),
		...
		reactloops.WithLoopUsagePrompt("Use when triaging many SSA risks in IRify: filter, bucket, and prioritize. For one risk at a time use ssa_risk_review. Irify attachments (runtime_id, program_name) sync at init. Use query_ssa_risk_overview with scalar params: limit, search, runtime_id, program_name, severity, risk_type."),
		reactloops.WithLoopOutputExample(`
* 总览当前项目风险：
  {"@action": "ssa_risk_overview", "human_readable_thought": "需要归纳并筛选 SSA 风险列表"}
* 多轮中按条件刷新数据库列表：
  {"@action": "query_ssa_risk_overview", "human_readable_thought": "用户收紧了 program 过滤", "limit": 40, "program_name": "demo"}
`),
	)
	if err != nil {
