...
<|GEN_CODE_END_{{ .nonce }}|>`, map[string]any{
	...
	loop, err := reactloops.NewReActLoop("aitag-single-loop", reactIns,
		...
		reactloops.WithPersistentInstruction("Generate code using <|GEN_CODE_CURRENT_NONCE|>code<|GEN_CODE_END_CURRENT_NONCE|> format"),
...
func TestExec_EdgeCase_RapidIterations(t *testing.T) {
	...
	reactIns, err := aireact.NewTestReAct(
		aicommon.WithAICallback(func(i aicommon.AICallerConfigIf, req *aicommon.AIRequest) (*aicommon.AIResponse, error) {
			...
			// 没有后续工作时 directly_answer 会在第一轮交付答复并自动收口.
			if iterCount == 1 {
	...
	if iterCount != 1 {
		t.Errorf("Expected 1 iteration (final directly_answer auto-finishes), got: %d", iterCount)
	}
