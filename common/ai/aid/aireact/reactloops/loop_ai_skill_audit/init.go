...
func init() {
	err := reactloops.RegisterLoopFactory(
		...
		reactloops.WithLoopUsagePrompt(`当用户需要对 Cursor Agent Skill（包含 SKILL.md 的目录）进行安全审计、安全扫描或安全检查时使用此流程。流程分三阶段：Phase 1 目录探索 → Phase 2 静态安全分析（检测恶意行为模式）→ Phase 3 报告生成。用户需提供 Skill 目录的绝对路径。`),
