...
根据 Phase1 侦察到的技术栈，自主决定 grep 关键词：
- 先搜用户输入来源（$_GET/$_POST/getParameter/c.Query 等），了解哪些变量接收用户数据
- 再搜 SQL 执行 Sink（函数名因框架/语言不同，根据技术栈选择）
...
根据技术栈选择对应的文件操作函数关键词 grep。重点关注文件路径的来源和净化方式。
...
根据技术栈选择 XML 解析器和 HTTP 客户端的对应关键词进行 grep。
...
根据技术栈选择 eval/exec 类函数关键词进行 grep，关注参数是否来自用户输入。
