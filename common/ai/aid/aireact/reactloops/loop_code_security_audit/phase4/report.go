...
// BuildReportLoop 构建 Phase 4 报告生成 Loop。
// 直接委托给 report_generating 子 loop，传入审计数据作为参考文件，
// 由专用报告写作 loop 完成 Markdown 报告的生成和磁盘写入。
func BuildReportLoop(r aicommon.AIInvokeRuntime, state *model.AuditState, opts ...reactloops.ReActLoopOption) (*reactloops.ReActLoop, error) {
	preset := []reactloops.ReActLoopOption{
		...
		reactloops.WithInitTask(func(loop *reactloops.ReActLoop, task aicommon.AIStatefulTask, op *reactloops.InitTaskOperator) {
			...
			// 确保 verified_vulns 覆盖全部 finding（Phase3 未 complete 时的最终兜底）
			state.DedupeVerifiedVulns()
			if filled := state.EnsureVerifyCoverage(); len(filled) > 0 {
				r.AddToTimeline("[PHASE4_VERIFY_GAP_FILL]",
					fmt.Sprintf("报告生成前补全 %d 个未验证 finding（uncertain）: %s",
						len(filled), strings.Join(filled, ", ")))
				log.Warnf("[CodeAudit/Phase4] Filled %d missing verify records before report", len(filled))
			}

			// 将 verified_vulns 持久化（确保报告 loop 能读到最新、完整文件）
			...
			// 读取写入结果，同步到 state，并 Go 层校验报告是否覆盖全部应写入的 finding
			if content, err := os.ReadFile(reportPath); err == nil && len(content) > 0 {
				reportText := string(content)
				repaired, missingIDs := model.RepairAuditReportCoverage(reportText, state.ReportableVerifiedVulns())
				if len(missingIDs) > 0 {
					if err := os.WriteFile(reportPath, []byte(repaired), 0o644); err != nil {
						log.Warnf("[CodeAudit/Phase4] Failed to write repaired report: %v", err)
					} else {
						reportText = repaired
				...
				log.Infof("[CodeAudit/Phase4] Report saved: %s (%d bytes)", reportPath, len(reportText))
			}

			stats2 := state.GetStats()
			emit.Phase4ReportComplete(loop, reportPath, stats2)
...
func buildAuditReportWritePrompt(state *model.AuditState, referenceFiles []string) string {
	stats := state.GetStats()
