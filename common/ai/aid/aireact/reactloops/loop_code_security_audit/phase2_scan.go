...
**当前迭代**: {{ .IterationCount }} / {{ .MaxIter }} | **本轮 Finding 数**: {{ .FindingsCount }}
...
func buildSingleCategoryScanLoop(r aicommon.AIInvokeRuntime, state *AuditState, category VulnCategory) (*reactloops.ReActLoop, error) {
	...
	// 迭代上限：SinkHints 数 * 4（每个 hint 预期 2-3 次 grep + 1 次 lock）+ 阶段B缓冲
	// SinkHints 不含关键词计数，用 hint 数量 * 4 作为阶段A预算，60 作为阶段B固定缓冲
	maxIter := (len(category.SinkHints)+1)*4 + 60
	if maxIter < 30 {
		maxIter = 30
	}
	if maxIter > 100 {
