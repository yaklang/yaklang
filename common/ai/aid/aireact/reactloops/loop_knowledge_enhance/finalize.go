...
// deliverFinalAnswerFallback 把已生成的报告以「参考资料 + DirectlyAnswer 对话答复」方式投递给用户。
// 行为：
...
func deliverFinalAnswerFallback(loop *reactloops.ReActLoop, invoker aicommon.AIInvokeRuntime, finalContent string) {
	...
	// 关键词: knowledge enhance, timeline injection, DirectlyAnswer
	...
	if compressed, err := invoker.CompressLongTextWithDestination(ctx, finalContent, userQuery, 10*1024); err == nil && strings.TrimSpace(compressed) != "" {
		...
		log.Warnf("knowledge enhance finalize: compress final report failed, fallback to raw content: %v", err)
	}
	invoker.AddToTimeline("enhanced_knowledge_content", timelineMaterial)

	// 关键词: knowledge enhance, DirectlyAnswer, WithReferenceMaterial
	...
	if userQuery != "" {
		...
		if err == nil && strings.TrimSpace(answer) != "" {
			...
			log.Infof("knowledge enhance finalize: delivered conversational answer via DirectlyAnswer (%d bytes)", len(answer))
			return
		}
		if err != nil {
		...
		log.Warnf("knowledge enhance finalize: empty user query, skip DirectlyAnswer and fallback to raw report")
	}

	deliverRawReportFallback(loop, invoker, finalContent)
}

// deliverRawReportFallback 在 DirectlyAnswer 不可用时，仍然以原始报告 markdown 形式投递。
// 仅由 deliverFinalAnswerFallback 在 DirectlyAnswer 失败时调用。
func deliverRawReportFallback(loop *reactloops.ReActLoop, invoker aicommon.AIInvokeRuntime, finalContent string) {
	taskIndex := ""
	if task := loop.GetCurrentTask(); task != nil {
