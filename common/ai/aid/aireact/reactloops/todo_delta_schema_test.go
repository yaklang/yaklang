...
func TestTodoDeltaIsSharedOptionalFieldNotStandaloneAction(t *testing.T) {
	...
	require.Contains(t, deltaDescription, "in the same action JSON that advances the work")
	require.Contains(t, deltaDescription, "custom TODO tags are read-only and ignored")
	...
	require.Contains(t, deltaDescription, "Closed IDs are immutable audit history")
	require.Contains(t, deltaDescription, "add a new TODO with a new ID")
	require.Contains(t, deltaDescription, "Never close or defer items merely to pass finish")
