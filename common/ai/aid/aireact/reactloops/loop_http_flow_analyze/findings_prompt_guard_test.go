...
func TestHTTPFlowEvidencePrompts_RequireDedicatedActionAndDirectAnswerPayload(t *testing.T) {
	...
	require.Contains(t, outputExample, "### 示例 A - 普通查询，不带 http_flow_evidence")
	require.Contains(t, outputExample, "### 示例 B - 发现可复用证据时，单独调用 record_http_flow_evidence")
	require.Contains(t, outputExample, "### 示例 C - http_flow_evidence 内容较长时，用 HTTP_FLOW_EVIDENCE AITag")
	require.Contains(t, outputExample, "<|HTTP_FLOW_EVIDENCE_CURRENT_NONCE|>")
	require.Contains(t, outputExample, "### 示例 D - 最终答复时必须给 answer_payload，不带 http_flow_evidence")
	require.Contains(t, outputExample, "### 示例 E - 最终答复较长时，用 FINAL_ANSWER AITag，不带 http_flow_evidence")
