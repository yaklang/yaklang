...
var loopActionDirectlyAnswerHTTPFuzztest = &reactloops.LoopAction{
	...
	NativeDescription: "在 answer_payload 中提供完整答复，包括 Markdown、列表或表格。已验证的风险必须先通过 generate_risk 保存；漏洞结论须包含证据链。普通任务答复后按 Current/TODO 状态续跑或 finish，安排后续工作时携带 todo_delta。",
