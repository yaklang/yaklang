...
func writeVerifyFrequencyExperimentReport(results []verifySimResult, baseSnapshotAge time.Duration, basePromptDelta, baseIterInterval int) error {
	...
	buf.WriteString("## 16.8 Fire 完成后基线对齐 (清零公平)\n\n")
	buf.WriteString("用户在多门交叉触发场景下提出补充诉求:\n\n")
	buf.WriteString("> \"在工具调用结束的 verification 中, 这个 gate 有很多层面, 但是很多层面都是交叉的.\n")
	buf.WriteString("> 有一个改动的需求是说, 当交叉出现的时候, 下一轮触发不应该太频繁, 比如说时间兜底\n")
	buf.WriteString("> 触发的时候, 迭代次数应该清零等操作. 并且迭代次数要及时清零, 这样整体才能公平公正.\"\n\n")
	buf.WriteString("背景与现状分析 (修复前):\n\n")
	buf.WriteString("- 自动路径 `MaybeVerifyUserSatisfaction` 触发 fire 后, 使用 fire **开始前** 计算的\n")
	buf.WriteString("  `currentSnapshot` 作为新基线落盘 (`setVerificationRuntimeSnapshot(currentSnapshot)`).\n")
	buf.WriteString("- 显式路径 `VerifyUserSatisfactionNow` 触发 fire 后, 使用 fire **结束后** 重新构造的\n")
	buf.WriteString("  实时 snapshot 作为新基线 (`setVerificationRuntimeSnapshot(r.buildVerificationRuntimeSnapshot(time.Now()))`).\n")
	buf.WriteString("- 两条路径的清零语义不一致. 在自动路径下, `prev.GeneratedAt` 比 fire 实际完成时间早\n")
	buf.WriteString("  AI 调用耗时 (常 5-30s), 等于把这段时间 \"白送\" 给时间门, 下一轮时间门 (180s) 会被\n")
	buf.WriteString("  提前到位. 多个门交叉触发后, 整体 verification 频率被不公平地推高.\n\n")
	buf.WriteString("修复策略:\n\n")
	buf.WriteString("- 自动路径在 fire 完成后, 改为用 `r.buildVerificationRuntimeSnapshot(time.Now())`\n")
	buf.WriteString("  重新构造实时 snapshot 作为新基线, 与显式路径行为完全对齐.\n")
	buf.WriteString("- 时间门 / iter 门 / 软 token 门冷静期下次判断都从 fire 真正完成那一刻起算,\n")
	buf.WriteString("  AI 调用耗时不再被任意一个门白送.\n")
	buf.WriteString("- 主循环 fire 期间是同步阻塞的, `currentIterationIndex` 与 `LoopPromptTokens` 在 fire\n")
	buf.WriteString("  期间不会变化, 所以这两个维度的值与 currentSnapshot 一致; 唯一发生变化的是\n")
	buf.WriteString("  `GeneratedAt` (从 fire 开始时间 → fire 完成时间).\n\n")
	buf.WriteString("仿真模型说明:\n\n")
	buf.WriteString("- `simVerifyController` 一直采用 \"fire 瞬时\" 假设 (`recordFire(now)` 与 fire 判断的\n")
	buf.WriteString("  `now` 是同一个时刻), 隐含的就是 \"fire 完成时刻 == 基线时刻\" 这一理想模型.\n")
	buf.WriteString("- 修复前生产代码偏离这一理想模型 (基线时刻是 fire 开始时刻), sim 报告里的 fired/lag\n")
	buf.WriteString("  数据其实低估了真实生产的频率.\n")
	buf.WriteString("- 修复后生产代码与 sim 模型对齐, 现有 sim 报告的 fired/lag 数据可以直接代表真实\n")
	buf.WriteString("  生产频率, 无需再叠加 AI 调用耗时的折算.\n\n")
	buf.WriteString("可见的端到端测试断言:\n\n")
	buf.WriteString("- `TestMaybeVerifyUserSatisfaction_BaselineRebuildAfterFire`: 模拟 60ms AI 延迟,\n")
	buf.WriteString("  断言 fire 完成后 `prev.GeneratedAt` 比 fire 开始时间晚至少 50ms, 反映真实结束时刻.\n")
	buf.WriteString("- `TestMaybeVerifyUserSatisfaction_TimeGateRefreshAfterFire`: 用时间门触发 fire,\n")
	buf.WriteString("  断言 fire 完成后 `prev.GeneratedAt` 远离旧 baseline 且晚于 fire 开始时间 50ms+,\n")
	buf.WriteString("  说明时间门基线已被同步推进到 fire 完成时刻.\n\n")

	buf.WriteString("## 16.9 风险与注意\n\n")
	...
	buf.WriteString("- 显式调用路径 (`VerifyUserSatisfactionNow` / `request_verification` action) 与自动路径\n")
