...
## 0. 核心原则：发现即上报留档（最高优先级，必须遵守）

`cybersecurity-risk` 是本流程的超重要常驻上报工具。越权漏洞或任何可能造成危害的安全风险一经一手数据确认（响应返回了他人/越权数据、管理操作被普通用户成功执行等），**第一时间**就用 `cybersecurity-risk` 落库，或把关键证据写入 output 结果文件留档（二者至少其一）。

- **每实例单独上报，禁止攒批**：越权测试天然会遍历大量 ID / 端点。每确认一个独立可越权实例就**立即上报一次**，绝不能等遍历完所有 ID / 端点再统一补录，更不能另起一个单独任务批量补报。
- **批量 IDOR 先报首个**：第 3.6 节的批量 ID 枚举，确认首个可越权实例后先上报，再继续枚举后续 ID；后续每命中一个新实例同样即时上报。
- 下文各 Phase 流程中写在末尾的 “报告: cybersecurity-risk (如果确认)” 步骤，都应理解为 “确认那一刻立即执行”，而不是 “攒到流程最后执行”。
- 留档前的探测/遍历可以继续，但**只要存在已确认却未落档的越权风险，就不得宣告任务完成或收尾**。
