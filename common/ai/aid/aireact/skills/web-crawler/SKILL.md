...
| 构建多页站点地图、发现链接/表单/参数 | `simple_crawler` | 先用较小 `reqs-max`/`max-depth`；只在新 URL 会影响测试时扩展 |
...
4. 普通爬虫不执行 JavaScript。若结果提示疑似 SPA，不要机械提高深度或请求数；改看一次渲染后 DOM，或分析 JS 资产。
...
- 明确记录普通爬虫不执行 JavaScript；没有发现 URL 不等于没有前端路由或 API
