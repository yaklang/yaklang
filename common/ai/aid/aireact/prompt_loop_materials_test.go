...
func TestPromptManager_AssembleLoopPrompt_SectionOrder(t *testing.T) {
	...
	require.Len(t, result.Sections[1].Children, 2)
	require.Equal(t, "section.semi_dynamic.skills_context", result.Sections[1].Children[0].Key)
	require.Equal(t, "section.semi_dynamic.tool_inventory", result.Sections[1].Children[1].Key)
