...
编辑 **`aicommon/prompts/prefix/high_static_section.txt`** 时须遵守与原 loop 版相同的约束（caller 无关、散文不写 schema enum 字面、不写 `<|TAG_<NONCE>|>` 形态、token 窗口等）。改完后可用：

```bash
grep -nE "require[-_]tool|directly[-_]answer|tool[-_]compose|finish[-_]exploration|enter[-_]focus[-_]mode|load[-_]capability|loading[-_]skills|load[-_]skill[-_]resources|change[-_]skill[-_]view[-_]offset|ask[-_]for[-_]clarification|abandon|@action|answer_payload|tool_require_payload" common/ai/aid/aicommon/prompts/prefix/high_static_section.txt
```
