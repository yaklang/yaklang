...
func TestExecuteToolWithCapture(t *testing.T) {
	...
	result1, err := tool.ExecuteToolWithCapture(context.Background(), params1, nil, nil, nil)
	if err != nil {
	...
	result2, err := tool.ExecuteToolWithCapture(context.Background(), params2, nil, nil, nil)
	if err != nil {
		t.Errorf("执行工具失败: %v", err)
	}
