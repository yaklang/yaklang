...
func BuildDynamicCapabilityTools(bridge Bridge) ([]*aitool.Tool, error) {
	...
	if err := factory.RegisterTool(
		...
		aitool.WithUsage("The runtime binds browser_ref to an instance explicitly attached by the user. Query only the relevant domain before browser.capability.call and follow paramsSchema exactly. Plaintext gateway, page cryptography, signatures, or encrypted payloads belong to domain=transform; network only observes requests and proxy only changes Chrome traffic routing. Never substitute use_browser/op=open for an attached instance."),
	...
	if err := factory.RegisterTool(
		...
		aitool.WithUsage("Call browser.capability.catalog for the relevant domain first. Pass the same browser_ref when several instances are attached. Use browser.tab.open or browser.tabs for navigation. For a plaintext gateway, query domain=transform and call browser.transform.profile.list first; use browser.transform.execute for an existing Profile, otherwise record a real operation, create a callable, call browser.profile.propose, then browser.profile.validate. A valid proposal becomes a short-lived local draft: stop and ask the user to confirm it in the extension; do not use proxy/network as a substitute and do not persist it. For QR/MFA/CAPTCHA call browser.handoff.request and wait. Never substitute use_browser/op=open."),
