...
### 6.1 messages 多角色透传（中等价值）

**现状**：aibalance 把多角色 messages 拍平为单条 user 消息。

**建议**：让 aibalance 保留 `bodyIns.Messages` 原结构透传到上游。这需要修改 `aispec.AIClient.Chat(string)` 接口，影响面较大。可作为长期重构方向，短期不做。
...
| 影响因素清单 | 7 项，按优先级排序 |
| 已落地小优化 | 亲和性路由（`PeekOrderedProvidersWithAffinity` + `BuildPromptAffinityKey`） |
| 单测 | 13 个新增，全部通过 |
| 回归测试 | aibalance 整包通过 |
| 未解决项 | 5 项，都标注了优先级和取舍考虑 |

亲和性路由是 **零侵入、零业务感知、保留所有兜底机制** 的最小改造。它把"命中率上界 1/N"提升到"命中率上界接近 1"，对 LiteForge 等"同 prompt 多次复发"场景收益最大。后续可通过 6.3、6.5 完成闭环观测。
