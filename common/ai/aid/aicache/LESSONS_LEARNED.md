...
- **【高优先级】消灭非主循环路径的 199-token 短高静态模板**:
  P0-LF1 已确认 28 次 199-token 告警全部来自 LiteForge 模板, 并已把
  `liteForgePromptTemplate` 的 high-static 段扩到 1561 tokens (见 §3.4
  "对策已落地"). **剩余: 1 次 732-token 告警 + 其他疑似非主循环入口**.
  任务:
  1. grep 全仓 `<|AI_CACHE_SYSTEM_high-static|>` 入口清单 + 各自模板路径.
  2. 跨 caller 稳定的内容统一接到 `high_static_section.txt` 或 LiteForge
     模板; caller-specific 的内容降档到 semi-dynamic.
