...
func HypotheticalAnswer(ctx context.Context, query string) (string, error) {
	...
	prompt, err := utils.RenderTemplate(`# 角色
你是一位资深的领域专家和顶级技术文档撰写者。
# 任务
根据用户提出的【问题】，生成一份简短但是结构完整、信息结构理想的文档。这份文档将被用作“黄金标准”，其目的是为了在海量信息中精准地检索到最相关的内容。因此，文档的质量、深度和全面性至关重要。
# 核心指令
1.  **深度与广度**: 不要满足于表面回答。深入挖掘问题的核心，覆盖其定义、关键原理、主要特性、应用场景，甚至相关概念的比较。**内容详实、信息密度高，是首要目标。**
2.  **结构化输出**: 根据问题的复杂性，自行组织内容的逻辑结构。例如，可以采用“**总-分-总**”或“**定义-要点-示例**”的结构，使用清晰的陈述句来构建一个逻辑连贯的小型文档。
3.  **事实性与中立性**: 内容必须基于公认的事实。**严禁编造信息或参数。** 如果信息不确定，应保持客观陈述，避免推测。
4.  **专业术语处理**: 如问题包含缩写/简称，必须先还原为规范全称。在首次出现时，建议采用“**全称（简称）**”的表述方式。
---
<|问题_{{ .nonce }}_START|>
{{ .query }}
<|问题_{{ .nonce }}_END|>
---
请立即以专家的身份，开始撰写这份专业、权威、详尽的文档。`, map[string]any{
		"nonce": utils.RandStringBytes(4),
		"query": query,
	})
	if err != nil {
