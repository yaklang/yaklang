...
func TestMUSTPASS_RAGSystem(t *testing.T) {
	...
	// 测试查询
	results, err := store.QueryWithPage("什么是RAG", 1, 5)
	assert.NoError(t, err)
	assert.Equal(t, 2, len(results))
...
func TestMUSTPASS_BigTextPlan(t *testing.T) {
	...
	// 创建RAG系统
	collectionMg, err := CreateCollection(consts.GetGormProfileDatabase(), "test", "test")
	assert.NoError(t, err)
	...
	t.Run("ChunkText Strategy", func(t *testing.T) {
		err := collectionMg.AddWithOptions("long_doc_1", longText)
		assert.NoError(t, err)
	...
	t.Run("Short Text No BigTextPlan", func(t *testing.T) {
		...
		err := collectionMg.AddWithOptions("short_doc", shortText)
		assert.NoError(t, err)
...
func Test_AddDocuments(t *testing.T) {
	testRag, err := GetCollection(consts.GetGormProfileDatabase(), "testRag")
	assert.NoError(t, err)
	...
	err = testRag.AddWithOptions("test", doc)
	assert.NoError(t, err)

	docs, err := testRag.List()
	assert.NoError(t, err)
