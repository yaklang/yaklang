	...
	tests := []struct {
		...
		},
		{
			...
			tool: newTool("multiTypeTool",
				...
				WithBool("booleanParam",
					Description("布尔参数"),
					Default(true),
					Required(),
				),
				WithStruct("objectParam", []PropertyOption{
					Description("对象参数"),
				}),
				WithNull("nullParam",
					Description("空参数"),
...
func TestComplexNestedStructures(t *testing.T) {
	complexTool := newTool("complexTool",
		...
		WithTool_Callback(testCallback),
		WithArrayEx("complexParam", []PropertyOption{
			Description("复杂参数"),
			Required(),
		},
			WithStruct("objectItem", []PropertyOption{
				Description("对象项"),
			}),
		),
	)
...
func TestDeepNestedStructures(t *testing.T) {
	deepNestedTool := newTool(
		...
		WithTool_Callback(testCallback),
		WithArrayEx("deepNested", []PropertyOption{
			Description("深层嵌套参数"),
		},
			WithArrayEx("first", []PropertyOption{
				Description("第一层嵌套"),
			},
				WithStringArrayEx("second", []PropertyOption{
					Description("第二层嵌套"),
				},
					Description("最内层参数"),
					Default("内层默认值"),
