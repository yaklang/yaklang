...
func TestMapThinkingEffortToConfig(t *testing.T) {
	tests := []struct {
		...
		{"high", true, "high"},
		{"HIGH", true, "high"},
		// xhigh/max and unknown values are downgraded to high
		{"xhigh", true, "high"},
		{"max", true, "high"},
		{"MAX", true, "high"},
		{"turbo", true, "high"},
