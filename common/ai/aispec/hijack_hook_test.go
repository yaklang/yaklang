...
// TestHijackHook_DispatchSync 验证 dispatchChatBaseHijackHooks 是同步的：
// 调用返回时所有 observer 一定已经执行完毕。
// 关键词: aispec, hijack hook dispatch sync
func TestHijackHook_DispatchSync(t *testing.T) {
	ResetChatBaseHijackHooksForTest()
	t.Cleanup(ResetChatBaseHijackHooksForTest)

	var got atomic.Int64
	RegisterChatBaseHijackHook(func(model, msg string) *ChatBaseHijackResult {
		got.Add(1)
		return nil
	})
	res := dispatchChatBaseHijackHooks("m", "hi")
...
// TestHijackHook_HijackResultPriority 多个 observer 都跑，最后一个 IsHijacked==true 胜出
// 关键词: aispec, hijack hook 优先级
func TestHijackHook_HijackResultPriority(t *testing.T) {
	ResetChatBaseHijackHooksForTest()
	t.Cleanup(ResetChatBaseHijackHooksForTest)

	RegisterChatBaseHijackHook(func(model, msg string) *ChatBaseHijackResult {
		return &ChatBaseHijackResult{
			IsHijacked: true,
			Messages: []ChatDetail{
	...
	res2 := dispatchChatBaseHijackHooks("m", "hi")
	require.NotNil(t, res2)
	assert.Equal(t, "third hijack", res2.Messages[0].Content,
		"last IsHijacked=true observer should win")
}

// TestHijackHook_PanicIsolation observer panic 不影响其他 observer 与主流程
// 关键词: aispec, hook panic 隔离
func TestHijackHook_PanicIsolation(t *testing.T) {
	ResetChatBaseHijackHooksForTest()
	t.Cleanup(ResetChatBaseHijackHooksForTest)
...
// TestHijackHook_NoObserverReturnsNil 未注册 observer 时返回 nil
// 关键词: aispec, hook 无注册返回 nil
func TestHijackHook_NoObserverReturnsNil(t *testing.T) {
	ResetChatBaseHijackHooksForTest()
	t.Cleanup(ResetChatBaseHijackHooksForTest)
	assert.Nil(t, dispatchChatBaseHijackHooks("m", "hi"))
}

// TestHijackHook_NilCallbackIgnored 注册 nil observer 不应 panic 也不应进入 list
// 关键词: aispec, hook nil 注册忽略
func TestHijackHook_NilCallbackIgnored(t *testing.T) {
	ResetChatBaseHijackHooksForTest()
	t.Cleanup(ResetChatBaseHijackHooksForTest)
	RegisterChatBaseHijackHook(nil)
	assert.Nil(t, dispatchChatBaseHijackHooks("m", "hi"))
}
...
// TestChatBase_HijackCorrelationIDPlumb 验证 hijack hook 写入的
// CorrelationID 能被 ChatBase 透传到 SSE 末帧 ChatUsage.MirrorCorrelationID,
// 让上层订阅者用稳定 ID 把 调试 dump (如 aicache dump) 与 token usage 精确 join,
// 修掉之前按数组下标对齐时遇到漏 callback 全部错位的归因 bug.
// 关键词: aispec ChatBase hook correlation id plumb, dump usage 精确对齐
func TestChatBase_HijackCorrelationIDPlumb(t *testing.T) {
	ResetChatBaseHijackHooksForTest()
	t.Cleanup(ResetChatBaseHijackHooksForTest)
...
`
	...
	RegisterChatBaseHijackHook(func(model, msg string) *ChatBaseHijackResult {
		return &ChatBaseHijackResult{CorrelationID: wantID}
	})
	...
	assert.Equal(t, wantID, captured.MirrorCorrelationID,
		"hijack result CorrelationID 必须被 ChatBase 透传到 ChatUsage")
	assert.Equal(t, 12, captured.PromptTokens, "原 usage 字段不能被透传逻辑破坏")
}

// TestChatBase_HijackCorrelationID_NoIDLeavesUsageUntouched hook 不写
// CorrelationID 时, ChatBase 不应包装 callback, ChatUsage.MirrorCorrelationID
// 保持空, 调用方原回调原样收到 usage.
// 关键词: aispec ChatBase hook correlation id 无 ID 不包装
func TestChatBase_HijackCorrelationID_NoIDLeavesUsageUntouched(t *testing.T) {
	ResetChatBaseHijackHooksForTest()
	t.Cleanup(ResetChatBaseHijackHooksForTest)
...
`
	...
	defer mu.Unlock()
	require.NotNil(t, captured)
	assert.Equal(t, "", captured.MirrorCorrelationID, "无关联 ID 时 usage 字段应保持空")
}
