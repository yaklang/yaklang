...
// ApplicationSettingsExtensionNew implements the TLS ALPS extension using the
// codepoint introduced by Chrome 133. It is intentionally kept separate from
// ApplicationSettingsExtension so existing 17513 fingerprints remain stable.
...
func (e *ApplicationSettingsExtensionNew) Len() int {
	bLen := 2 + 2 + 2
	for _, protocol := range e.SupportedProtocols {
		bLen += 1 + len(protocol)
	}
	return bLen
}
...
func (e *ApplicationSettingsExtensionNew) Read(b []byte) (int, error) {
	...
	b[0] = byte(utlsExtensionApplicationSettingsNew >> 8)
	b[1] = byte(utlsExtensionApplicationSettingsNew & 0xff)
	lengths := b[2:]
	b = b[6:]
...
func (e *ApplicationSettingsExtensionNew) Write(b []byte) (int, error) {
	...
	for !protoList.Empty() {
		...
		if !protoList.ReadUint8LengthPrefixed(&protocol) || protocol.Empty() {
			return 0, errors.New("unable to read ALPN extension data")
		}
		protocols = append(protocols, string(protocol))
	}
	e.SupportedProtocols = protocols
	return fullLen, nil
}
