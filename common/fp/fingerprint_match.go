...
func (f *Matcher) MatchWithContext(ctx context.Context, host string, port int, options ...ConfigOption) (result *MatchResult, err error) {
	...
	proto, port := utils.ParsePortToProtoPort(port)
	addr := utils.HostPort(host, port)

	if f.Config.IsFiltered(host, port) {
		return nil, utils.Errorf("[IGNORE] %v is filtered by servicescan.excludeHosts/Ports", addr)
	}
	...
	serviceFirst := func() (*MatchResult, error) {
		...
		if serviceName != "" && serviceName != "tcp" && serviceName != "ssl" && !utils.MatchAllOfRegexp(serviceName, "(?i)http") {
			return result, nil
		}
	...
	if config.OnlyEnableWebFingerprint {
		log.Debugf("web-detect first for: %v", utils.HostPort(host, port))
		matchResult, err = webFirst()
	} else if config.DisableWebFingerprint {
		log.Debugf("service-detect first for: %v", utils.HostPort(host, port))
		matchResult, err = serviceFirst()
	} else {
		// 使用预定义的端口范围来决定扫描策略
		webPortsFilter := utils.ParseStringToPorts(webPorts)
		if utils.IntArrayContains(webPortsFilter, port) {
			log.Debugf("web-detect first for: %v", utils.HostPort(host, port))
			matchResult, err = webFirst()
		} else {
			// 默认策略，可以根据实际情况调整
	...
	if matchResult.State == OPEN && matchResult.Fingerprint != nil {
		matchResult.Fingerprint.TLSInspectResults, _ = netx.TLSInspectTimeout(utils.HostPort(host, port), 5)
	}
