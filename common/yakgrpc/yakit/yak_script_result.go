...
func YakScriptType(db *gorm.DB) (req []*TagAndTypeValue, err error) {
	db = db.Raw(`SELECT count(*) as count, type as value FROM yak_scripts where is_history = '0' and ignored = '0' and type != 'packet-hack' GROUP BY type order by count desc;`)
	db = db.Scan(&req)
	if db.Error != nil {
