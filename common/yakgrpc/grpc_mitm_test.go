...
func TestGRPCMUSTPASS_MITM_GM_IsHttps(t *testing.T) {
	t.Run("GM_IsHttps", func(t *testing.T) {
		...
		for {
			...
			gmPacket := lowhttp.ReplaceHTTPPacketHeader([]byte("GET /gm-ishttps-"+token+" HTTP/1.1\r\nHost: placeholder\r\n\r\n"), "Host", utils.HostPort(mockGMHost, mockGMPort))
			_, err = yak.NewScriptEngine(10).ExecuteEx(`
...
`, map[string]any{"packet": gmPacket, "proxy": proxy, "h": mockGMHost, "p": mockGMPort})
			...
			httpsPacket := lowhttp.ReplaceHTTPPacketHeader([]byte("GET /https-ishttps-"+token+" HTTP/1.1\r\nHost: placeholder\r\n\r\n"), "Host", utils.HostPort(mockHttpsHost, mockHttpsPort))
			_, err = yak.NewScriptEngine(10).ExecuteEx(`
rsp, req = poc.HTTP(string(getParam("packet")), poc.proxy(getParam("proxy")), poc.host(getParam("h")), poc.port(getParam("p")), poc.https(true))~
`, map[string]any{"packet": httpsPacket, "proxy": proxy, "h": mockHttpsHost, "p": mockHttpsPort})
			require.NoError(t, err)

			httpPacket := lowhttp.ReplaceHTTPPacketHeader([]byte("GET /http-ishttps-"+token+" HTTP/1.1\r\nHost: placeholder\r\n\r\n"), "Host", utils.HostPort(mockHost, mockPort))
			_, err = yak.NewScriptEngine(10).ExecuteEx(`
...
`, map[string]any{"packet": httpPacket, "proxy": proxy, "h": mockHost, "p": mockPort})
			...
			for retry := 0; retry < 5; retry++ {
				_, gmFlows, err = yakit.QueryHTTPFlow(db, &ypb.QueryHTTPFlowRequest{SearchURL: "/gm-ishttps-" + token})
				require.NoError(t, err)
				if len(gmFlows) > 0 {
					break
				}
			...
			_, httpsFlows, err = yakit.QueryHTTPFlow(db, &ypb.QueryHTTPFlowRequest{SearchURL: "/https-ishttps-" + token})
			require.NoError(t, err)
			...
			// HTTP flow 加重试；SourceType: "mitm" 限定只取 MITM 保存的 flow，排除 poc.HTTP/lowhttp 保存的 flow（后者 IsHTTPS 来自客户端到代理的连接，plain HTTP 时为 false）
			for retry := 0; retry < 5; retry++ {
				_, httpFlows, err = yakit.QueryHTTPFlow(db, &ypb.QueryHTTPFlowRequest{
					SearchURL:  "/http-ishttps-" + token,
					SourceType: "mitm",
				})
				require.NoError(t, err)
				if len(httpFlows) > 0 {
