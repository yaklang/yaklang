...
`, map[string]any{"packet": httpPacket, "proxy": proxy, "h": mockHost, "p": mockPort})
		...
		_, gmFlows, err := yakit.QueryHTTPFlow(consts.GetGormProjectDatabase(), &ypb.QueryHTTPFlowRequest{SearchURL: "/gm-ishttps-" + token})
		require.NoError(t, err)
		...
		_, httpsFlows, err := yakit.QueryHTTPFlow(consts.GetGormProjectDatabase(), &ypb.QueryHTTPFlowRequest{SearchURL: "/https-ishttps-" + token})
		require.NoError(t, err)
		...
		_, httpFlows, err := yakit.QueryHTTPFlow(consts.GetGormProjectDatabase(), &ypb.QueryHTTPFlowRequest{SearchURL: "/http-ishttps-" + token})
		require.NoError(t, err)
