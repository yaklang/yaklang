...
func TestGRPCMUSTPASS_MITM_HotPatch_DropWebsocketUpgrade(t *testing.T) {
	ctx, cancel := context.WithCancel(utils.TimeoutContextSeconds(20))
	defer cancel()

	host, port := utils.DebugMockEchoWs("hotpatch-drop-ws")
	mitmPort := utils.GetRandomAvailableTCPPort()
	client, err := NewLocalClient()
	require.NoError(t, err)
	stream, err := client.MITM(ctx)
...
`,
			...
			packet := fmt.Sprintf(`GET /hotpatch-drop-ws HTTP/1.1
Host: %s
Origin: http://%s
Connection: Upgrade
Upgrade: websocket
