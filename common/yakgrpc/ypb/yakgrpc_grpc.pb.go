...
const (
	...
	Yak_StartSSEMCP_FullMethodName                                = "/ypb.Yak/StartSSEMCP"
)
...
type YakClient interface {
	...
	// MCP
	StartSSEMCP(ctx context.Context, in *StartMCPRequest, opts ...grpc.CallOption) (Yak_StartSSEMCPClient, error)
}
...
func (c *yakClient) StartSSEMCP(ctx context.Context, in *StartMCPRequest, opts ...grpc.CallOption) (Yak_StartSSEMCPClient, error) {
	stream, err := c.cc.NewStream(ctx, &Yak_ServiceDesc.Streams[74], Yak_StartSSEMCP_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &yakStartSSEMCPClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type Yak_StartSSEMCPClient interface {
	Recv() (*StartMCPResponse, error)
	grpc.ClientStream
}

type yakStartSSEMCPClient struct {
	grpc.ClientStream
}

func (x *yakStartSSEMCPClient) Recv() (*StartMCPResponse, error) {
	m := new(StartMCPResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
...
type YakServer interface {
	...
	// MCP
	StartSSEMCP(*StartMCPRequest, Yak_StartSSEMCPServer) error
...
func (UnimplementedYakServer) StartSSEMCP(*StartMCPRequest, Yak_StartSSEMCPServer) error {
	return status.Errorf(codes.Unimplemented, "method StartSSEMCP not implemented")
}
...
func _Yak_StartSSEMCP_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StartMCPRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(YakServer).StartSSEMCP(m, &yakStartSSEMCPServer{stream})
}

type Yak_StartSSEMCPServer interface {
	Send(*StartMCPResponse) error
	grpc.ServerStream
}

type yakStartSSEMCPServer struct {
	grpc.ServerStream
}

func (x *yakStartSSEMCPServer) Send(m *StartMCPResponse) error {
	return x.ServerStream.SendMsg(m)
}
...
var Yak_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "StartSSEMCP",
			Handler:       _Yak_StartSSEMCP_Handler,
			ServerStreams: true,
		},
	},
