...
func (s *Server) execRequest(req *ypb.ExecRequest, moduleName string, ctx context.Context, handler func(
	...
	for _, p := range req.GetParams() {
		...
		case "__yakit_plugin_names__":
			fp, err := os.CreateTemp(os.TempDir(), "yakit-plugin-selector-*.txt")
			if err != nil {
	...
	if req.GetRunnerParamRaw() != "" {
		paramsExisted := strings.Join(params, " ")
	...
	log.Info("start to run yak code/script...")
	var (
		scriptPath = req.GetScriptPath()
		scriptCode = req.GetScript()
		scriptId   = req.GetScriptId()
	)
	createTempYakScript := func(code string) (string, error) {
		f, err := os.CreateTemp("", "yakit-code-*.yak")
		if err != nil {
	...
	if scriptPath != "" {
		...
		scriptPath, err = createTempYakScript(script.Content)
		defer os.Remove(scriptPath)
	}

	if scriptPath == "" {
		return utils.Errorf("invalid exec request")
	}

	cmd := exec.CommandContext(
		ctx, yakEnginePath,
		append([]string{scriptPath}, params...)...)
