...
func TestGRPCMUSTPASS_MITMV2_HotPatch_Drop(t *testing.T) {
...
func TestGRPCMUSTPASS_MITMV2_HotPatch_Dangerous_FuzzTag(t *testing.T) {
	ctx, cancel := context.WithCancel(utils.TimeoutContextSeconds(5))
	defer cancel()
...
func TestGRPCMUSTPASS_MITMV2_HotPatch_BeforeRequest_AfterRequest(t *testing.T) {
...
`, map[string]any{
			...
			require.Len(t, data.ManualHijackList, 1)
			hijackTask := data.ManualHijackList[0]
			require.Equal(t, hijackTask.Status, Hijack_Status_Response)
			// send packet
...
func TestGRPCMUSTPASS_MITMV2_HotPatch_Output(t *testing.T) {
	ctx, cancel := context.WithCancel(utils.TimeoutContextSeconds(100))
	defer cancel()
...
func TestGRPCMUSTPASS_MITMV2_HotPatch_HijackSaveHTTPFlow(t *testing.T) {
...
`
	check := func(t *testing.T, http2 bool) {
		...
		}, func(mitmClient ypb.Yak_MITMV2Client, msg *ypb.MITMV2Response) {
			if msg.GetCurrentHook && len(msg.GetHooks()) > 0 {
