...
func TestP0ScorecardsCovered(t *testing.T) {
	...
	for _, item := range ProtocolRoadmap {
		if item.Priority != priP0 {
			continue
		}
		sc, ok := ResolveP0Scorecard(item.Name)
		if !ok {
		...
		if item.Status == stDone {
			g := sc.Grade()
			if g != "A" && g != "B" {
				t.Errorf("P0 %q is done but grade %s total %d (need A/B)", item.Name, g, sc.Total())
			}
			if !sc.GatesOK() {
				t.Errorf("P0 %q is done but a hard gate failed", item.Name)
			}
		}
	}
	require.Empty(t, leftover, "P0 protocols missing scorecards: %v", leftover)
