...
本轮以 `5b6cbe1f8` 为原始性能基线，仍使用单批 11 条消息、1,421 B、完整字段和元数据 JSON。最终程序为 `35607643c`，五次独立 3 秒交替窗口的中位数为：单 worker 1.694 → 0.843 ms/批（2.008859 倍），10 worker 0.882 → 0.466 ms/批（1.892561 倍）。原 `aa8d5b36c` 候选曾达到主指标 2.04 倍，但 AllJoyn 大计数回退持续超过 5%；修复后，该负载在两个设置下的正式耗时变化分别为 +0.56% / +0.69%。错误 PWD 的首轮未达标结果、独立路径成本、全部慢值及撤回实验完整保留，不混合不同环境或不同候选的窗口。

完整回归（包含 P0）和相关 race 已通过。各阶段对照、其他负载、源码与程序身份、所有原始值及 CI 状态见 [完整报告](reports/hotpath-20260907/README.md)。该报告作为本轮验收记录；本页后续旧数据继续标注各自基线，历史加速比例不相加，不外推为网卡无丢包带宽。
