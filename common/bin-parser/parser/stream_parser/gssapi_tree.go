...
// The temporary bounded read and entirely new tree commit together. Decode
// failure leaves the caller's existing children, metadata and reader intact.
func parseGSSAPIToken(node *base.Node, process func(*base.Node) (func(bool), error)) error {
	bits, bounded, err := parseLengthByLengthConfig(node)
	if err != nil {
		return err
	}
	if !bounded || bits%8 != 0 || bits < 4*8 || bits > gssapiMaxBytes*8 {
		return fmt.Errorf("gssapi: explicit byte token boundary must be 4..65535 bytes")
	}
	raw, err := base.NewNodeTreeWithConfig(node.Cfg, "GSS-API staged token", "raw", node.Ctx)
	if err != nil {
		return err
	}
	...
	fields, info, err := decodeGSSAPIToken(value.([]byte))
	if err != nil {
		return err
	}
	...
	var fill func(*base.Node, []gssapiField) error
	fill = func(parent *base.Node, fields []gssapiField) error {
		for _, f := range fields {
			if f.Start < 0 || f.End < f.Start || uint64(f.End) > bits/8 {
				return fmt.Errorf("gssapi: invalid decoded span")
			}
			var child *base.Node
			if f.Type == "" {
				...
				if f.Type != "raw" && f.Type != "uint8" {
					return fmt.Errorf("gssapi: unsupported decoded type")
				}
