...
// The complete exact HTTP record is staged before any child or metadata is
// committed. No HTTP context keys, namespace maps or caller config are mutated.
func parseWinRMRecord(node *base.Node, process func(*base.Node) (func(bool), error)) error {
	bits, bounded, err := parseLengthByLengthConfig(node)
	if err != nil {
		return err
	}
	if !bounded || bits == 0 || bits%8 != 0 || bits > 8<<20 {
		return fmt.Errorf("winrm: explicit 1..1048576 byte record boundary required")
	}
	raw, err := base.NewNodeTreeWithConfig(node.Cfg, "WinRM staged record", "raw", node.Ctx)
	if err != nil {
		return err
	}
	...
	wire, ok := value.([]byte)
	if !ok {
		return fmt.Errorf("winrm: staged record is not raw")
	}
	...
	var fill func(*base.Node, []winrmField) error
	fill = func(parent *base.Node, fields []winrmField) error {
		for _, f := range fields {
			if f.Start < 0 || f.End < f.Start || uint64(f.End)*8 > bits {
				return fmt.Errorf("winrm: invalid field span")
			}
			var child *base.Node
			if f.Type == "" {
				...
				if f.Decoded {
					child.Cfg.SetItem("winrm decoded value", f.Value)
					child.Cfg.SetItem("out", "return node.Cfg.GetItem(\"winrm decoded value\")")
				}
			}
