...
func parseZlibJSONRecord(node *base.Node, process func(*base.Node) (func(bool), error)) error {
	bits, bounded, err := parseLengthByLengthConfig(node)
	if err != nil {
		return err
	}
	if !bounded || bits%8 != 0 || bits < 12*8 || bits > (4+zlibJSONMaxCompressed)*8 {
		return fmt.Errorf("zlib-json: explicit byte request boundary must be 12..65540 bytes")
	}
	raw, err := base.NewNodeTreeWithConfig(node.Cfg, "Zlib JSON staged record", "raw", node.Ctx)
	if err != nil {
		return err
	}
	...
	fields, info, err := decodeZlibJSONRecord(value.([]byte))
	if err != nil {
		return err
	}
	...
	var fill func(*base.Node, []zlibJSONField) error
	fill = func(parent *base.Node, fields []zlibJSONField) error {
		for _, f := range fields {
			if f.Start < 0 || f.End < f.Start || uint64(f.End) > bits/8 {
				return fmt.Errorf("zlib-json: invalid decoded span")
			}
			var child *base.Node
			if f.Type == "" {
				...
				if f.Type != "raw" && f.Type != "string" && f.Type != "uint8" && f.Type != "uint16" && f.Type != "uint32" && f.Type != "uint64" {
					return fmt.Errorf("zlib-json: unsupported decoded type")
				}
