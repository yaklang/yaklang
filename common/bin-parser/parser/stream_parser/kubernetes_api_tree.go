...
// A complete bounded request is staged before committing fields or metadata.
func parseKubernetesAPIRequest(node *base.Node, process func(*base.Node) (func(bool), error)) error {
	bits, bounded, err := parseLengthByLengthConfig(node)
	if err != nil {
		return err
	}
	if !bounded || bits == 0 || bits%8 != 0 || bits > 65536*8 {
		return fmt.Errorf("kubernetes api: explicit 1..65536 byte record boundary required")
	}
	raw, err := base.NewNodeTreeWithConfig(node.Cfg, "Kubernetes API staged request", "raw", node.Ctx)
	if err != nil {
		return err
	}
	...
	wire, ok := value.([]byte)
	if !ok {
		return fmt.Errorf("kubernetes api: staged request is not raw")
	}
	...
	var fill func(*base.Node, []kubernetesAPIField) error
	fill = func(parent *base.Node, fields []kubernetesAPIField) error {
		for _, f := range fields {
			if f.Start < 0 || f.End < f.Start || uint64(f.End)*8 > bits {
				return fmt.Errorf("kubernetes api: invalid field span")
			}
			var child *base.Node
			if f.Type == "" {
				...
				if f.Decoded {
					child.Cfg.SetItem("kubernetes api decoded value", f.Value)
					child.Cfg.SetItem("out", "return node.Cfg.GetItem(\"kubernetes api decoded value\")")
				}
			}
