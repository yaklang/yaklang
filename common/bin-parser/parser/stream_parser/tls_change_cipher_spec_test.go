...
func TestTLSChangeCipherSpecBridgeTransactions(t *testing.T) {
	for offset := uint64(0); offset < 8; offset++ {
		for _, outcome := range []string{"commit", "invalid", "short", "callback"} {
			t.Run(fmt.Sprintf("%d/%s", offset, outcome), func(t *testing.T) {
				...
				root.Cfg.SetItem(CfgLength, offset+48)
				p := &DefParser{}
				require.NoError(t, p.OnRoot(root))
				n.Cfg.SetItem(CfgLength, uint64(48))
				...
				calls, finishes := 0, 0
				err := parseTLSChangeCipherSpecRecord(n, func(raw *base.Node) (func(bool), error) {
					calls++
				...
				if outcome == "commit" {
					require.NoError(t, err)
					giopBridgeAssertTree(t, n, offset, offset+48)
					require.Equal(t, offset+48, ctx.GetUint64("pointer"))
				} else {
					require.Error(t, err)
	...
	for _, bits := range []uint64{0, 1, 7, 9, 16, 47, 49, 56, 16390 * 8} {
		root := giopBridgeInlineRoot(t, "Package:\n  Message: {}\n")
		n := root.Children[0].Children[0]
		n.Cfg.SetItem(CfgLength, bits)
		err := parseTLSChangeCipherSpecRecord(n, func(*base.Node) (func(bool), error) { t.Fatal("invalid boundary read input"); return nil, nil })
		require.Error(t, err)
	}
}
