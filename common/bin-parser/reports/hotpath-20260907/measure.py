...
def run(artifacts, mode, tag, rounds):
    ...
    def execute(label, workers, expression, pair):
        ...
        env = dict(os.environ, GOMAXPROCS=str(workers))
        if mode == 'digests':
        ...
        for line in content.splitlines():
            ...
            if match:
                ...
                measurements.append({'name': re.sub(r'-\d+$', '', match.group(1)), 'iterations': int(match.group(2)),
        ...
        row = {'sequence': counter, 'label': label, 'commit': s['commit'], 'pair': pair,
               'workers': workers, 'expression': expression, 'command': args, 'cwd': str(PARSER),
