...
func parseSFScanConfig(c *cli.Context) (res *ssaCliConfig, err error) {
	...
	codeSourceKind := cfg.GetCodeSourceKind()
	...
	// 如果提供了 target 但没有设置 codeSourceKind，提示用户使用配置文件
	if targetPath != "" && codeSourceKind == "" {
		return nil, utils.Errorf("codeSourceKind is required when using --target. Please use --config to provide a configuration file generated by 'SSA 项目探测.yak', or ensure the config file contains code_source.kind")
	}
