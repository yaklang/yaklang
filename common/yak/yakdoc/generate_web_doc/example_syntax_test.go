...
// TestYakitDBExampleSyntax 对 yakit、db 两个标杆库的真实生成产物，逐个抽取示例代码做 antlr
// 语法/编译检查(进程内、不联网、不执行)。这是文档"内容质量"的强约束：任何示例语法错误即红。
// 其它库暂不强约束(可后续逐步纳入)，避免一次性卡住全量。
// 关键词: 示例语法校验, antlr FormattedAndSyntaxChecking, yakit/db 标杆
func TestYakitDBExampleSyntax(t *testing.T) {
	...
	for _, name := range []string{"yakit", "db"} {
