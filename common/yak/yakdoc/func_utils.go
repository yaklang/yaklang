...
func handleParams(funcRefType reflect.Type, buf []byte, typ *ast.FuncType) (params []*Field) {
	if typ.Params == nil {
		return nil
	}
...
func handleResults(funcRefType reflect.Type, buf []byte, typ *ast.FuncType) (results []*Field) {
	if typ.Results == nil {
		return nil
	}
...
func FuncToFuncDecl(f interface{}, libName string, overideName string) (*FuncDecl, error) {
	...
	for _, theFunc := range funcs {
		...
		if decl != nil && decl.Type != nil && decl.Type.Params != nil {
			params = handleParams(funcRefType, buf, decl.Type)
		}

		// 获取返回值
		if decl != nil && decl.Type != nil && decl.Type.Results != nil {
			results = handleResults(funcRefType, buf, decl.Type)
		}
			...
			for _, elt := range value.Elts {
				...
				if ok {
					...
					if funcDecl, ok := decl.(*ast.FuncDecl); ok {
						params = handleParams(funcRefType, buf, funcDecl.Type)
						results = handleResults(funcRefType, buf, funcDecl.Type)
						found = true
						break
					}
					...
					if specs, ok := decl.(*ast.ValueSpec); ok {
						...
						if ok {
							params = handleParams(funcRefType, buf, funcLit.Type)
							results = handleResults(funcRefType, buf, funcLit.Type)
							found = true
							break
						}
					}
				...
				if funcDecl, ok := decl.(*ast.FuncDecl); ok {
					...
					params = handleParams(funcRefType, buf, typ)
					results = handleResults(funcRefType, buf, typ)
					found = true
					break
				}
	...
	// 生成vscode参数补全
	paramAutoCompletionStr = strings.Join(paramsStr, ", ")
