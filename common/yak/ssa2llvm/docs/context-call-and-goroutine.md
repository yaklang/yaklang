...
- stdlib dispatcher：`yak_runtime_dispatch(void* ctx)`
- goroutine 启动入口：`yak_runtime_spawn(void* ctx)`
...
普通 Yak 函数、extern hook、fallback 符号都走 `compileInvokeCallable`：
...
stdlib 不直接链接成一堆导出符号，而是走统一 dispatcher：
...
- 普通 callable 异步调用仍走 `compileInvokeCallable`
- stdlib 异步调用走 `compileAsyncStdlibDispatchCall`
- 两条路径最后都不直接 `call`，而是调用 `yak_runtime_spawn(ctx)`
...
1. `yak_runtime_spawn(ctx)` 调用 `yak_ctx_root_add(ctx)`；
2. root 节点由 Boehm GC 管理；
3. goroutine 结束后调用 `yak_ctx_root_remove(handle)`；
4. 整个异步执行期间，`ctx` 及其 `Roots` 都保持可达。

### 5.2 goroutine 内部如何决定执行什么

`yak_runtime_spawn` 起 goroutine 后，会根据 `ctx.kind` 分发：

- `KindCallable`：调用 `yak_invoke_callable(fn, ctx)`；
- `KindDispatch`：调用 `yak_runtime_dispatch(ctx)`；
