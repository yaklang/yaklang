...
func buildModulePackageMap(modules []string) map[string][]string {
	knownPaths := map[string][]string{
		// The poc module's heavy transitive closure (lowhttp, net/http,
		// schema/DB, quic, utls, etc.) is only needed by scripts that use poc.
		// Assigning all of it to .modtext.poc lets lld drop the whole closure
		// for scripts that do not use poc, instead of leaving it in the
		// non-splittable packed .text.
		"poc": {
		...
		// The ssa module's language frontends (typescript/java/php/python
		// parsers, antlr4, go2ssa/c2ssa, goja) are only needed by scripts that
		// use ssa. Keep them in .modtext.ssa so lld can drop the whole frontend
		// stack for scripts that do not use ssa.
		"ssa": {
