...
// For Python, we filter out non-Python files similar to Java implementation.
func (*SSABuilder) FilterPreHandlerFile(path string) bool {
	extension := filepath.Ext(path)
	// Filter out common non-code files
	fileList := []string{".jpg", ".png", ".gif", ".jpeg", ".css", ".js", ".avi", ".mp4", ".mp3", ".pdf", ".doc", ".php", ".go", ".java"}
	for _, ext := range fileList {
		if extension == ext {
			return false
		}
	}
