...
// Source: §8.1 General
type_
    : {p.IsTypeParameterName()}? type_parameter
    | {p.IsValueTypeName()}?     value_type
    | {p.IsReferenceTypeName()}? reference_type
...
non_nullable_reference_type
    : {p.IsDelegateTypeName()}?  delegate_type
    | {p.IsInterfaceTypeName()}? interface_type
    | {p.IsClassTypeName()}?     class_type
    | array_type
    | 'dynamic'
    ;
...
pattern
    : {p.IsDeclarationPatternAhead()}? declaration_pattern
    | {p.IsConstantPatternAhead()}?    constant_pattern
    | var_pattern
    | {p.IsPositionalPatternAhead()}? positional_pattern
    | property_pattern
    | discard_pattern
    ;
...
primary_expression
    ...
    | parenthesized_expression
    // From: member_access
        | primary_expression '.' identifier type_argument_list? { p.AsMemberAccess(localctx); }
        | predefined_type '.' identifier type_argument_list? { p.AsMemberAccess(localctx); }
        | qualified_alias_member '.' identifier type_argument_list? { p.AsMemberAccess(localctx); }
    // From: null_conditional_member_access
    	| primary_expression '?' '.' identifier type_argument_list? (null_forgiving_operator? dependent_access)* { p.AsNullConditionalMemberAccess(localctx); }
    // From: invocation_expression
        | primary_expression '(' argument_list? ')' { p.AsInvocationExpression(localctx); }
    // From: element_access and pointer_element_access (unsafe code support)
        | primary_expression '[' argument_list ']' { p.AsElementAccess(localctx); } { p.ElementAccessSemanticCheck(localctx); }
    // From: null_conditional_element_access
        | primary_expression '?' '[' argument_list ']' (null_forgiving_operator? dependent_access)* { p.AsNullConditionalElementAccess(localctx); } { p.ElementAccessSemanticCheck(localctx); }
    | this_access
    | base_access
    // From: post_increment_expression
        | primary_expression '++' { p.AsPostIncrementExpression(localctx); }
    // From: post_decrement_expression
        | primary_expression '--' { p.AsPostDecrementExpression(localctx); }
    // | null_forgiving_expression
        | primary_expression null_forgiving_operator { p.AsNullForgivingExpression(localctx); }
    | array_creation_expression
    | object_creation_expression
    | delegate_creation_expression
    | anonymous_object_creation_expression
    ...
    // From: pointer_member_access     // unsafe code support
        | primary_expression '->' identifier type_argument_list? { p.AsPointerMemberAccess(localctx); }
...
cast_expression
    : {p.IsCastExpressionAhead()}? '(' type_ ')' unary_expression
    ;
...
// Original:
//   local_variable_declaration
//       : {p.IsImplicitlyTypedLocalVariable()}? implicitly_typed_local_variable_declaration
//       | {p.IsExplicitlyTypedLocalVariable()}? explicitly_typed_local_variable_declaration
//       | {p.IsExplicitlyTypedRefLocalVariable()}? explicitly_typed_ref_local_variable_declaration
//       ;
local_variable_declaration
    : ref_kind? local_variable_type { p.BeginVariableDeclaration(); }
      local_variable_declarator
      (',' local_variable_declarator {p.IsLocalVariableDeclaration()}?)*
    ;
...
// Original (explicit): identifier { p.OnVariableDeclarator(); } ('=' local_variable_initializer)?
local_variable_declarator
    : identifier { p.OnVariableDeclarator(); }
...
//       : type_ { p.BeginVariableDeclaration(); }
...
//       ;
non_ref_local_variable_declaration
    : local_variable_type { p.BeginVariableDeclaration(); }
      local_variable_declarator
      (',' local_variable_declarator {p.IsLocalVariableDeclaration()}?)*
    ;
...
using_alias_directive
    : 'using' identifier '=' namespace_or_type_name ';'
      { p.OnUsingAliasDirective(); }
    ;

// Source: §14.5.3 Using namespace directives
using_namespace_directive
    : 'using' namespace_name ';'
      { p.OnUsingNamespaceDirective(); }
    ;
...
class_base
    : {p.IsClassBaseInterfaceList()}? ':' interface_type_list
    | {p.IsClassBaseClassType()}?     ':' class_type
...
field_declaration
    : attributes? field_modifier* type_ { p.BeginVariableDeclaration(); }
