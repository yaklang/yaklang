...
// ║ 
Interpolated_Regular_String_Mid
    : Interpolated_Regular_String_Element+ { l.WrapToken(); }
    ;
...
// ║ 
Regular_Interpolation_Format
    : ':' Interpolated_Regular_String_Element+ { p.LookAheadIsRBrace1() }? { p.PeekModeIsIrsCont() }? { l.WrapToken(); }
    ;
...
// ║ 
Interpolated_Verbatim_String_Mid
    : Interpolated_Verbatim_String_Element+ { l.WrapToken(); }
    ;
...
// ║ 
Verbatim_Interpolation_Format
    : ':' Interpolated_Verbatim_String_Element+ { p.LookAheadIsRBrace1() }? { p.PeekModeIsIvsCont() }? { l.WrapToken(); }
    ;
...
Interpolated_Regular_Solitary_LBrace
    : { p.LookAheadIsNotLBrace2() }? '{' -> type(TK_LBRACE), pushMode(DEFAULT_MODE)
    ;
