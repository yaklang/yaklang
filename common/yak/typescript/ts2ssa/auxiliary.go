...
func (b *builder) SwitchFunctionBuilder(s *ssa.StoredFunctionBuilder) func() {
	...
	// BuildSyntaxBlock 结束后切换到 EndBlock）。store.CurrentBlock() 是注册时的值快照，
	...
	if currentBlock := s.CurrentBlock(); currentBlock != nil {
		b.FunctionBuilder.CurrentBlock = currentBlock
	}
	return func() {
		b.LoadBuilder(t)
	}
	...
	namespaceObj.AddLazyBuilder(func() {
		...
		if container != nil {
			for _, pair := range ssa.GetLastWinsMemberPairs(container) {
				namespaceObj.RegisterStaticMember(pair.KeyString(), pair.Member)
				cnt++
			}
		}
