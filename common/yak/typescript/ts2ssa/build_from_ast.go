...
func (b *builder) VisitSourceFile(sourcefile *ast.SourceFile) interface{} {
	...
	if b.PreHandler() {
		...
		if sourcefile.Statements != nil {
			...
			for _, statement := range sourcefile.Statements.Nodes {
				if ast.IsFunctionDeclaration(statement) || ast.IsVariableStatement(statement) {
		...
		for exportName, exportedObjectName := range b.namedExports {
			exportedObjectVal := b.PeekValue(exportedObjectName)
			if exportedObjectVal != nil {
				lib.SetExportValue(exportName, exportedObjectVal)
			}
		}
		return nil
	}
...
func (b *builder) VisitExportAssignment(node *ast.ExportAssignment) interface{} {
	...
	if !node.IsExportEquals { // export default
		if variable := b.VisitLeftValueExpression(node.Expression); variable != nil {
			b.namedExports["default"] = variable.GetName()
		}
	} else {
		if variable := b.VisitLeftValueExpression(node.Expression); variable != nil {
...
func (b *builder) VisitFunctionDeclaration(node *ast.FunctionDeclaration) interface{} {
	...
	var isExport, isDefault bool
	if modifierFlags := node.Modifiers(); modifierFlags != nil {
		isExport = modifierFlags.ModifierFlags&ast.ModifierFlagsExport != 0
		isDefault = modifierFlags.ModifierFlags&ast.ModifierFlagsDefault != 0
	}
	if isExport {
		if !isDefault {
			b.namedExports[funcName] = funcName
		} else {
			b.namedExports["default"] = funcName
		}
