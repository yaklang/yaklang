...
func searchMembersInKeyMatchMode(value *Value, inst ssa.Value, check func(*Value) bool, add func(*Value)) {
	...
	// 检查是否真的有 overlay：需要确保 overlay 有效且有多个 layer
	// 避免在非 overlay 场景下误入 overlay 逻辑
	// 单个 layer 的情况应该使用普通逻辑，避免不必要的开销
	if value.ParentProgram != nil && value.ParentProgram.overlay != nil {
		overlay := value.ParentProgram.GetOverlay()
		// 只有当 overlay 存在且至少有 2 个 layer 时，才使用 overlay 逻辑
	...
	// 没有 overlay 或只有一个 layer，使用原来的逻辑：直接从当前 instruction 获取成员
