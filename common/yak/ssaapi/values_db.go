...
func (s *saveValueCtx) recursiveSaveValue(value *Value) error {
	...
	for _, pred := range value.Predecessors {
		if !s.theEdgeShouldVisit(value, pred.Node, s.ProgramName, ssadb.EdgeType_Predecessor) {
			continue
		}
		var step int64
		var label string
		...
		edge := ssadb.CreatePredecessorEdge(s.ProgramName, value.GetId(), pred.Node.GetId(), step, label)
