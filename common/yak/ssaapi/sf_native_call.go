...
特殊地，在 Java 中，会尽可能使用全限定类名，例如 com.alibaba.fastjson.JSON, 也会尽可能包含 sca 版本`),
	...
	registerNativeCall(NativeCall_UAF, nc_func(nativeCallUAF), nc_desc(`Use-After-Free 检测：基于堆对象生命周期分析。Double-free 视为 UAF 子类一并返回。用法：*<uaf()> as $uaf 扫描全程序；或 $ptr<uaf()> 仅返回与选定指针/free 实参相关的命中。当前支持 C（malloc/calloc/realloc + free），过程内 + 简单拷贝别名 + 浅 free-param summary。`))
	registerNativeCall(NativeCall_NPD, nc_func(nativeCallNPD), nc_desc(`空指针解引用（NPD）检测：独立于 UAF。路径上跟踪指针空性（Null/MaybeNull），在 *p / p->f 等成员解引用处报警。用法：*<npd()> as $npd。不将 call 实参视为解引用。当前支持 C 过程内。`))
}
