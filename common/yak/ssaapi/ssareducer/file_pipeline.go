...
const (
	FileStatusSuccess FileStatus = iota
	FileStatusFsError
	FileStatusParseError
	FileStatusSkipped
)
	...
	parseASTPipe := pipeline.NewPipeWithStore[*FileContent, *FileContent](
		ctx, bufSize, func(fileContent *FileContent, store *utils.SafeMap[any]) (*FileContent, error) {
			...
			if err != nil {
				...
				fileContent.Status = FileStatusParseError
			} else if ast == nil {
				fileContent.Status = FileStatusSkipped
			} else {
				fileContent.Status = FileStatusSuccess
			}
			return fileContent, nil
		},
		initWorker,
