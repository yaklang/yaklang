...
var wired2 = `package com.macro.mall.controller;
...
func TestBigJavaFile2(t *testing.T) {
	var astCost time.Duration
	var memCost time.Duration
	var dbCost time.Duration
	ssatest.ProfileJavaCheck(t, wired2, func(mem bool, prog *ssaapi.Program, start time.Time) error {
		if prog == nil {
			astCost = time.Since(start)
			return nil
		}
