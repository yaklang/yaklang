...
type ValueGraph struct {
	*dot.Graph
	...
	marshaledValue *omap.OrderedMap[int, struct{}] // node-id ->  ssaapi.value
...
func NewValueGraph(v ...*Value) *ValueGraph {
	...
	g := &ValueGraph{
		...
		marshaledValue: omap.NewOrderedMap(map[int]struct{}{}),
...
func (g *ValueGraph) _marshal(selfID int, value *Value) {
	if _, ok := g.marshaledValue.Get(selfID); ok {
		return
	}
	g.marshaledValue.Set(selfID, struct{}{})

	currentMarshaled := g.marshaledValue
	for _, node := range value.GetDependOn() {
		g.marshaledValue = currentMarshaled.Copy()
