...
/*
Context ReaderWriter
*/

type ctxReaderWriter struct {
	io.Reader
	io.Writer
...
func (crw *ctxReaderWriter) Read(buf []byte) (int, error) {
	buf2 := make([]byte, len(buf))

	c := make(chan ioret, 1)

	go func() {
		n, err := crw.Reader.Read(buf2)
		c <- ioret{n, err}
		close(c)
	}()
...
func (crw *ctxReaderWriter) Write(buf []byte) (int, error) {
	buf2 := make([]byte, len(buf))
	copy(buf2, buf)
...
func NewReaderWriter(ctx context.Context, r io.ReadWriter) *ctxReaderWriter {
	return &ctxReaderWriter{ctx: ctx, Reader: r, Writer: r}
}

/*
Context wrapper Connection
*/
type ctxConn struct {
	net.Conn
	ctx context.Context
...
func (cc *ctxConn) Read(buf []byte) (int, error) {
	buf2 := make([]byte, len(buf))
...
func (w *ctxConn) Write(buf []byte) (int, error) {
	buf2 := make([]byte, len(buf))
	copy(buf2, buf)
...
func NewConn(ctx context.Context, conn net.Conn) net.Conn {
