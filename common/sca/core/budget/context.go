...
// Ensure binds one shared default state at an operation boundary. Calling From
// repeatedly with an unbound background context cannot accumulate a budget.
func Ensure(ctx context.Context) context.Context {
	if ctx == nil {
		ctx = context.Background()
	}
	if _, ok := ctx.Value(key{}).(*State); ok {
		return ctx
	}
	return Bind(ctx, Limits{})
}
...
// Insert reserves a string key and small value, including low-occupancy bucket
// growth, before the corresponding map write. Larger values need their own charge.
func (s *State) Insert(key string) error {
	return s.Add(0, SizeOfString(key)+128)
}
