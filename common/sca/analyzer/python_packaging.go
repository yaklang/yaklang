...
func (a pythonPackagingAnalyzer) Analyze(afi AnalyzeFileInfo) ([]*dxtypes.Package, error) {
	...
	case statusEgg:
		...
		if err := reserveZipDirectory(fi.LazyFile.Context(), fi.LazyFile, realFileInfo.Size()); err != nil {
			return nil, err
		}
		...
		for _, vf := range zr.File {
			...
			if err = policy.Working(128 << 10); err != nil {
				return nil, err
			}
			r, err := vf.Open()
			if err != nil {
			...
			data, err := textdecode.ReadRaw(fi.LazyFile.Context(), r, policy.Limits.MaxFileBytes)
			r.Close()
			if err != nil {
				return nil, err
			}
