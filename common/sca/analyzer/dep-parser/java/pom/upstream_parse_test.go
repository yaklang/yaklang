...
func TestPom_Parse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := range got {
				got[i] = types.Library{Name: got[i].Name, Version: got[i].Version, License: got[i].License}
			}

			// B08: source-defined declaration and exact-coordinate corrections.
			switch tt.name {
			case "offline mode":
				tt.want = append(tt.want, types.Library{Name: "com.example:child", Version: "${revision}-${changelist}"})
			case "local repository", "remote repository":
				tt.want = append(tt.want, types.Library{Name: "org.example:example-provided", Version: "999"})
			case "dependencyManagement prefers child properties":
				for i := range tt.want {
					if tt.want[i].Name == "org.example:example-api" {
						tt.want[i].Version = "2.0.0"
					}
				}
			case "inherit parent dependencies":
				tt.want = []types.Library{{Name: "com.example:child", Version: "${revision}-${changelist}", License: "Apache 2.0"}}
			case "transitive parents":
				for i := range tt.want {
					if tt.want[i].Name == "org.example:example-child" {
						tt.want[i].License = "Apache 2.0"
					}
				}
			case "version requirement":
				tt.want = append(tt.want, types.Library{Name: "org.example:example-api", Version: "(,1.0]"})
			}
