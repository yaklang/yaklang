...
func (p *Parser) parseArtifact(filePath string, size int64, r types.ReadSeekerAt) ([]types.Library, []types.Dependency, error) {
	log.Debugf("Parsing Java artifacts... file=%s", filePath)

	zr, err := zip.NewReader(r, size)
	if err != nil {
		return nil, nil, utils.Errorf("zip error: %w", err)
	}
	...
	for _, fileInJar := range zr.File {
		...
		case filepath.Base(fileInJar.Name) == "pom.properties":
			...
			if err != nil {
				return nil, nil, utils.Errorf("failed to parse %s: %w", fileInJar.Name, err)
			}
		...
		case filepath.Base(fileInJar.Name) == "MANIFEST.MF":
			...
			if err != nil {
				return nil, nil, utils.Errorf("failed to parse MANIFEST.MF: %w", err)
			}
		case isArtifact(fileInJar.Name):
			innerLibs, _, err := p.parseInnerJar(fileInJar, filePath) // TODO process inner deps
			if err != nil {
				log.Debugf("Failed to parse %s: %s", fileInJar.Name, err)
				continue
			}
	...
	if p.offline {
		...
		if !manifestProps.Valid() {
			log.Debugf("Unable to identify POM in offline mode file=%s", fileName)
			return libs, nil, nil
		}
	...
	if err == nil {
		...
		return nil, nil, utils.Errorf("failed to search by SHA1: %w", err)
	}

	log.Debugf("No such POM in the central repositories file=%s", fileName)
	...
	if err == nil {
		log.Debugf("POM was determined in a heuristic way file=%s artifact=%s", fileName, fileProps.String())
		libs = append(libs, fileProps.Library())
	} else if !errors.Is(err, ArtifactNotFoundErr) {
		return nil, nil, utils.Errorf("failed to search by artifact id: %w", err)
	}
...
func (p *Parser) parseInnerJar(zf *zip.File, rootPath string) ([]types.Library, []types.Dependency, error) {
	fr, err := zf.Open()
	if err != nil {
		return nil, nil, utils.Errorf("unable to open %s: %w", zf.Name, err)
	}

	f, err := os.CreateTemp("", "inner")
	if err != nil {
		return nil, nil, utils.Errorf("unable to create a temp file: %w", err)
	}
	defer func() {
		f.Close()
		os.Remove(f.Name())
	...
	if _, err = io.Copy(f, fr); err != nil {
		return nil, nil, utils.Errorf("file copy error: %w", err)
	}
	...
	if err != nil {
		return nil, nil, utils.Errorf("failed to parse %s: %w", zf.Name, err)
	}
...
func (p *Parser) searchBySHA1(r io.ReadSeeker, filePath string) (Properties, error) {
	if _, err := r.Seek(0, io.SeekStart); err != nil {
		return Properties{}, utils.Errorf("file seek error: %w", err)
	}

	h := sha1.New()
	if _, err := io.Copy(h, r); err != nil {
		return Properties{}, utils.Errorf("unable to calculate SHA-1: %w", err)
	}
...
func parsePomProperties(f *zip.File, filePath string) (Properties, error) {
	file, err := f.Open()
	if err != nil {
		return Properties{}, utils.Errorf("unable to open pom.properties: %w", err)
	}
	defer file.Close()
	...
	if err = scanner.Err(); err != nil {
		return Properties{}, utils.Errorf("scan error: %w", err)
	}
	return p, nil
}
...
func parseManifest(f *zip.File) (manifest, error) {
	file, err := f.Open()
	if err != nil {
		return manifest{}, utils.Errorf("unable to open MANIFEST.MF: %w", err)
	}
	defer file.Close()
	...
	if err = scanner.Err(); err != nil {
		return manifest{}, utils.Errorf("scan error: %w", err)
	}
	return m, nil
}
...
func (m manifest) determineGroupID() (string, error) {
	...
	default:
		return "", utils.Error("no groupID found")
	}
...
func (m manifest) determineArtifactID() (string, error) {
	...
	default:
		return "", utils.Error("no artifactID found")
	}
...
func (m manifest) determineVersion() (string, error) {
	...
	default:
		return "", utils.Error("no version found")
	}
