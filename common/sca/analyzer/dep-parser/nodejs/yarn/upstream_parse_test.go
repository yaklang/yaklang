...
func TestParse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantDeps != nil {
				// Independent inspection of the optionalDependencies blocks in the
				// pinned realworld lockfile: the former parser dropped all three.
				if strings.Contains(tt.file, "realworld") {
					for _, id := range []string{"jsonfile@2.4.0", "jsonfile@4.0.0", "klaw@1.3.1"} {
						tt.wantDeps = append(tt.wantDeps, types.Dependency{ID: id, DependsOn: []string{"graceful-fs@4.1.15"}})
					}
				}
				sortDeps(deps)
