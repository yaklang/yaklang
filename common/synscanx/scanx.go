...
func NewScannerx(ctx context.Context, sample string, config *SynxConfig) (*Scannerx, error) {
	...
	if config.netInterface == "" {
		iface, gatewayIP, srcIP, err = netutil.Route(time.Second*2, sample)
		if err != nil {
		...
		if err != nil {
			return nil, errors.Errorf("get gateway failed: %s", err)
		}
	}
	...
	for _, host := range filteredHosts {
		for _, port := range filtedPorts {
			...
			s.OnSubmitTask(func(h string, p int) {
				s.config.submitTaskCallback(utils.HostPort(h, p))
			})
