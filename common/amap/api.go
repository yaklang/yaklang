...
func (c *Client) doRequest(ctx context.Context, path string, params url.Values, v interface{}) error {
	...
	// Build URL
	reqURL := fmt.Sprintf("%s%s?%s", c.config.BaseURL, path, params.Encode())
	...
	if resp.StatusCode != http.StatusOK {
		...
		return fmt.Errorf("amap API request failed with status %d: %s", resp.StatusCode, string(body))
	}
...
type ReverseGeocodingResponse struct {
	BaseResponse
	RegeoCodes []*RegeoCodeResult `json:"regeocodes"`
}
...
func (c *Client) Bicycling(ctx context.Context, request BicyclingRequest) (*DirectionResponse, error) {
	params := url.Values{}
	params.Set("origin", request.Origin.String())
	params.Set("destination", request.Destination.String())
	...
	response := &DirectionResponse{}
	err := c.doRequest(ctx, "/v3/direction/bicycling", params, response)
	return response, err
}
...
type BusLine struct {
	...
	Duration  string `json:"duration"`
	Polyline  string `json:"polyline"`
	Departure string `json:"departure_stop"`
	Arrival   string `json:"arrival_stop"`
