...
// BicyclingResultEx contains bicycling route information
type BicyclingResultEx struct {
	Distance     string
	Duration     string
	Instructions []string
}
...
func WalkingPlan(origin, destination string, options ...AmapConfigOption) (*DirectionResponse, error) {
	originGeocodes, err := Geocode(origin, options...)
	if err != nil {
		return nil, err
	}
...
func BicyclingPlan(origin, destination string, options ...AmapConfigOption) (*BicyclingResultEx, error) {
	originGeocodes, err := Geocode(origin, options...)
	if err != nil {
		return nil, err
	}
	...
	if len(resp.Route.Paths) == 0 {
		return nil, fmt.Errorf("no bicycling routes found")
	}

	path := resp.Route.Paths[0]
	instructions := make([]string, len(path.Steps))
	for i, step := range path.Steps {
		instructions[i] = step.Instruction
	}

	return &BicyclingResultEx{
...
func Distance(origin, destination string, options ...AmapConfigOption) (*DistanceResponse, error) {
	config := NewConfig(options...)
...
func SearchNearbyPOI(longitude, latitude float64, keywords string, options ...AmapConfigOption) (*SearchPOIResultEx, error) {
	config := NewConfig(options...)
...
func GetAOIBoundary(aoiID string, options ...AmapConfigOption) (*AOIBoundaryResultEx, error) {
	config := NewConfig(options...)
...
func GetWeather(cityCode string, forecast bool, options ...AmapConfigOption) (*WeatherResponse, error) {
	config := NewConfig(options...)
