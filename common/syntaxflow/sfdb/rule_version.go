...
func GenerateRuleID(language string, cve int, tags []string) string {
	prefix := -1
	switch language {
	case string(consts.General):
		prefix = 0
	case string(consts.GO):
		prefix = 1
	...
	// Generate a random number between 0 and 999 for the index
	index := rand.Intn(1000) // xxx

	if cve == 0 {
		cve = rand.Intn(1000) + 5000 // 5xxx
	}

	return fmt.Sprintf("%01d%04d%03d", prefix, cve, index)
