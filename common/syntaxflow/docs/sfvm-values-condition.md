...
- 输入 `Values` 拍平为 leaf
- 每个 leaf 并发执行
- wrapper 统一负责结果收集，以及在 condition scope 内的 anchor 传播
...
- 普通模式下：整个输入当成一组
- condition scope 下：按当前 source slot 自动分组
...
## 5. `?{}` / `?()` 语义：统一都是 condition-scope（anchor 开启）
