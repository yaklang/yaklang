# SyntaxFlow risk type taxonomy

`taxonomy.json` is the producer-owned dictionary for built-in rule risk types.
It separates canonical machine keys, Chinese labels, navigation categories,
and explicitly reviewed legacy aliases. Built-in source uses canonical keys;
historical and custom values remain readable through aliases and raw fallback.
...
5. Keep canonical names and aliases stable. Bump `version` when changing
...
need local databases. The compiled-corpus check covers rule defaults, alert
overrides, and effective types for findings. Libraries may remain unclassified.

```bash
go test ./common/syntaxflow/sfrisk -count=1
go test ./common/schema -run '^TestSSARiskTaxonomyPreservesRawValueAndHash$' -count=1
