...
func TestBindHTTPRequestToDownstreamPreservesBufferedPipelineOnClose(t *testing.T) {
	...
	case <-request.Context().Done():
	case <-time.After(2 * time.Second):
		t.Fatal("request context was not canceled after buffered pipeline closed")
	}
