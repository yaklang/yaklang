...
func (c *ServerConfig) servePortal(conn net.Conn) {
	...
	if err != nil {
		...
		// Aggregate models from providers
...
func (c *ServerConfig) HandlePortalRequest(conn net.Conn, request *http.Request, uriIns *url.URL) {
	...
	} else if uriIns.Path == "/portal/update-model-meta" && request.Method == "POST" {
		c.handleUpdateModelMeta(conn, request)
	} else if uriIns.Path == "/portal/totp-settings" {
		c.serveTOTPSettingsPage(conn)
	} else if uriIns.Path == "/portal/refresh-totp" && request.Method == "POST" {
...
func (c *ServerConfig) handleUpdateModelMeta(conn net.Conn, request *http.Request) {
	...
	// Save to DB
	if err := SaveModelMeta(reqBody.ModelName, reqBody.Description, reqBody.Tags); err != nil {
	...
	c.logInfo("Successfully updated metadata for model %s", reqBody.ModelName)
	c.writeJSONResponse(conn, http.StatusOK, map[string]interface{}{
		"success": true,
