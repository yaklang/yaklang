...
func main() {
	err := App.Run(os.Args)
	if err != nil {
		log.Errorf("app run failed: %s", err)
		return
	}
}
func createApp() *cli.App {
	...
	app := cli.NewApp()
	app.Name = "yso"
	app.Description = "yak-yso 是一个用于生成 java payload 的命令行工具"
	app.UsageText = `format: yak yso [options] [gadget] [type] [params]
...
`
	app.UsageText += fmt.Sprintf("all gadget:%s\n\n", strings.Join(gadgetNames, ","))
	app.UsageText += fmt.Sprintf("all class:\n%s\n", strings.Join(classInfos, ""))
	app.UsageText += fmt.Sprintf("all transform chain:\n%s\n", strings.Join(transformChainInfos, ""))
	app.UsageText += `example:
