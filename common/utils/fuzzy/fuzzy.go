// Origin: https://github.com/lithammer/fuzzysearch
...
func RankFindSSDeepStability(source string, targets []string) Ranks {
	return rankFindEx(source, targets, noopTransformer(), func(s1, s2 string) float64 {
		f, err := utils.CalcSSDeepStability([]byte(s1), []byte(s2))
		if err != nil {
			f = math.MaxFloat64
		}
		return f
...
func RankFindSimHashStability(source string, targets []string) Ranks {
	return rankFindEx(source, targets, noopTransformer(), func(s1, s2 string) float64 {
		f, err := utils.CalcSimHashStability([]byte(s1), []byte(s2))
		if err != nil {
			f = math.MaxFloat64
		}
		return f
...
func rankFindEx(source string, targets []string, transformer transform.Transformer, f func(string, string) float64) Ranks {
	sourceT := stringTransform(source, transformer)
	if f == nil {
