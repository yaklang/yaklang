/*
Reference: https://github.com/google/shlex
...
const (
	...
	ansiCQuotingEscapingState                   // we are within a ansi-c quoted string that supports escaping ($'...')
...
func newDefaultClassifier() tokenClassifier {
	...
	t.addRuneClass(escapingQuoteRunes, doubleQuoteRuneClass)
	t.addRuneClass(ansiCRunes, ansiCRuneClass)
	t.addRuneClass(nonEscapingQuoteRunes, singleQuoteRuneClass)
...
func (t *Tokenizer) scanStream() (*Token, error) {
	...
	for {
		...
		case startState: // no runes read yet
			...
			case ansiCRuneClass:
				tokenPrefix = nextRune
			case singleQuoteRuneClass:
				tokenType = WordToken
				switch tokenPrefix {
				case '$':
					state = ansiCQuotingEscapingState
				default:
					state = quotingState
				}
		...
		case ansiCQuotingEscapingState:
			switch nextRuneType {
			case eofRuneClass:
				err = fmt.Errorf("EOF found when expecting closing quote")
				token := &Token{
			...
			case singleQuoteRuneClass:
				state = inWordState
				if unquoted, err := utils.UnquoteANSICWithQuote(string(value), 0); err == nil {
					value = []rune(unquoted)
				} else {
					err = fmt.Errorf("Error unquoting ansi-c string: %v", err)
					token := &Token{
						tokenType: tokenType,
						value:     string(value),
					}
...
// Support ANSI-C quoting ($'...'), double quoting ("..."), and single quoting ('...').
