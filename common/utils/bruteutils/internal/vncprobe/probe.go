...
func Probe(ctx context.Context, dialer Dialer, o Options) (r Result) {
	...
	defer stop()
	defer func() {
		if r.Err == nil || classifiedLoginErr(r.Err) {
			return
		}
		if ctx.Err() != nil {
			r.Err = ctx.Err()
	...
	sec, err := s.negotiateSecurity(o.UnauthOnly)
	if err != nil {
		r.Err = err
		return r
	}
	used, err := s.doSecurity(sec, o.Password, o.UnauthOnly)
	if err != nil {
		r.Err = err
		return r
	}
...
func (s *session) negotiateSecurity(unauth bool) (uint8, error) {
	if s.minor < 7 {
		var raw uint32
		if err := binary.Read(s.conn, binary.BigEndian, &raw); err != nil {
			return 0, s.ioErr("security type", err)
		}
		if raw == 0 {
			_, _ = s.readReason()
	...
	if err := binary.Read(s.conn, binary.BigEndian, &n); err != nil {
		return 0, s.ioErr("security count", err)
	}
	if n == 0 {
		_, _ = s.readReason()
		return 0, ErrProtocolMismatch
	...
	if _, err := io.ReadFull(s.conn, types); err != nil {
		return 0, s.ioErr("security types", err)
	}
	chosen := pickSecurity(types, unauth)
	if chosen == 0 {
...
func pickSecurity(types []byte, unauth bool) uint8 {
