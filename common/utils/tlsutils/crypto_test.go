...
// TestRSAEncryptDecryptLongPlaintext 测试超过单块长度的明文加密解密（分块，兼容 JS 常见实现）
func TestRSAEncryptDecryptLongPlaintext(t *testing.T) {
	t.Parallel()
	// 2048-bit key: PKCS1v15 单块最多 245 字节，OAEP(SHA256) 约 190 字节
	pubKey := `
-----BEGIN RSA PUBLIC KEY-----
MIIBCgKCAQEAs1pvFYNQpPSPbshg6F7ZaR31s14iwKacmG5vvQp8Xq38tBJaC8WP
Pcuv0/66hFe5zfE5pl+yUK37mjBaRMmcO2C0ommeVcAm3yKZ3x6FHVaj/YM6z+F3
...
sUT/btRaRcGKtGXKLpSU0513RZc1mrsAOwblzfKcSjT8hv4sztAC
-----END RSA PRIVATE KEY-----`
	longPlain := make([]byte, 500)
	for i := range longPlain {
		longPlain[i] = byte('A' + (i % 26))
	}

	enc1, err := Pkcs1v15Encrypt([]byte(pubKey), longPlain)
	require.NoError(t, err)
	dec1, err := Pkcs1v15Decrypt([]byte(priKey), enc1)
	require.NoError(t, err)
	require.Equal(t, longPlain, dec1, "PKCS1v15 long plaintext mismatch")

	enc2, err := PkcsOAEPEncryptWithHash([]byte(pubKey), longPlain, sha256.New())
	require.NoError(t, err)
	dec2, err := PkcsOAEPDecryptWithHash([]byte(priKey), enc2, sha256.New())
	require.NoError(t, err)
	require.Equal(t, longPlain, dec2, "OAEP long plaintext mismatch")
}
