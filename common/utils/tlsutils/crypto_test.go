...
func TestPemSignRSA(t *testing.T) {
	t.Parallel()
	pubKey := `
-----BEGIN RSA PUBLIC KEY-----
MIIBCgKCAQEAs1pvFYNQpPSPbshg6F7ZaR31s14iwKacmG5vvQp8Xq38tBJaC8WP
...
-----END RSA PRIVATE KEY-----`
	...
	// PKCS1v15 + SHA-256
	sig1, err := PemSignRSA([]byte(priKey), msg, "PKCS1v15", "SHA-256")
	if err != nil {
		t.Fatal(err)
	}
	if err = PemVerifyRSA([]byte(pubKey), msg, sig1, "PKCS1v15", "SHA-256"); err != nil {
		t.Fatal(err)
	}
	if err = PemVerifyRSA([]byte(pubKey), []byte("wrong"), sig1, "PKCS1v15", "SHA-256"); err == nil {
		t.Fatal("verify should fail for wrong message")
	}

	// PSS + SHA-256
	sig2, err := PemSignRSA([]byte(priKey), msg, "PSS", "SHA-256")
	if err != nil {
		t.Fatal(err)
	}
	if err = PemVerifyRSA([]byte(pubKey), msg, sig2, "PSS", "SHA-256"); err != nil {
		t.Fatal(err)
	}
}
