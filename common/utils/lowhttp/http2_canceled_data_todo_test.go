...
func TestH2CanceledStreamReturnsConnectionWindowTODO(t *testing.T) {
	// TODO(H2): Returning connection-level credit for DATA received after a
	// stream is removed is correct flow-control behavior, but needs broader
	// compatibility coverage before changing the current read loop.
	t.Skip("TODO(H2): return connection WINDOW_UPDATE for DATA received after stream cancellation")
