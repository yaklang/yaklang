...
func findProcessName(network string, ip netip.Addr, srcPort int) (uint32, string, error) {
	once.Do(func() {
		if err := initSearcher(); err != nil {
			log.Errorf("Initialize PROCESS-NAME failed: %s", err.Error())
			log.Warnf("All PROCESS-NAME rules will be skipped")
			return
		}
	})
...
func (s *searcher) Search(buf []byte, ip netip.Addr, port uint16, isTCP bool) (uint32, error) {
	...
	for i := s.headSize; i+itemSize <= len(buf); i += itemSize {
		...
		case flag&0x1 > 0 && isIPv4:
			// ipv4
			srcIP = IpToAddr(buf[inp+s.ip : inp+s.ip+4])
		case flag&0x2 > 0 && !isIPv4:
			// ipv6
			srcIP = IpToAddr(buf[inp+s.ip-12 : inp+s.ip+4])
		default:
			continue
		}
