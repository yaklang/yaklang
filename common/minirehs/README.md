...
```text
cum%   函数
35.8%  github.com/dlclark/regexp2.(*runner).execute      <- regexp2 兜底 (2~3 条 always-on URL 规则)
...
12.9%  minirehs.(*mvsNFA).existsInAssert                  <- 零宽断言 NFA
 8.9%  regexp2/syntax.CharSet.CharIn                      <- regexp2 兜底
...
```text
flat%  函数
57.4%  github.com/dlclark/regexp2.getRunes               <- 每次把整段输入转 []rune (每 pattern·每报文一次)
...
3. **定位的零分配化 (内存 ~15%)** —— `findLocFrom` 的起点数组/结果切片改用 scratch 复用缓冲。
