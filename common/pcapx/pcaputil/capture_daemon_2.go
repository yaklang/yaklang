...
var captureDaemonCache = ttlcache.New[string, *daemonCache](
...
func init() {
	captureDaemonCache.OnEviction(func(ctx context.Context, reason ttlcache.EvictionReason, item *ttlcache.Item[string, *daemonCache]) {
		if reason == ttlcache.EvictionReasonCapacityReached {
			fmt.Println("cap ", item.Key(), item.Value())
		}
		if reason == ttlcache.EvictionReasonExpired {
			log.Errorf("captureDaemonCache %s is expired", item.Key())
			item.Value().handler.Close()
		}
		if reason == ttlcache.EvictionReasonDeleted {
			fmt.Println("deleted "+item.Key(), item.Value())
		}
	})
	go captureDaemonCache.Start()
}
...
func getDaemonCache(key string) (*daemonCache, bool) {
	if captureDaemonCache.Has(key) {
		return captureDaemonCache.Get(key).Value(), true
	}
	return nil, false
}
