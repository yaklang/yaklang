...
func (f *binFlow) detectDirection(dir int, w []byte) {
	...
	case "iec104":
		f.protocol, f.iec104 = "iec104", &binIEC104{pending: map[uint16]string{}}
	case "dnp3":
...
func (f *binFlow) consumeSession(dir int, e *ProtocolEvent, result map[string]any) error {
	...
	case "iec104":
		e.Session, err = f.iec104.consume(e.Raw)
	case "dnp3":
