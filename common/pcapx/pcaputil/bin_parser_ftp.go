...
func ftpKnownCommand(cmd string) bool {
	...
	case "USER", "PASS", "ACCT", "CWD", "CDUP", "QUIT", "REIN", "PORT", "PASV", "TYPE", "STRU", "MODE",
		...
		"PROT", "EPSV", "EPRT", "SIZE", "MDTM", "XPWD", "CLNT", "MLST", "MLSD":
		return true
	}
	return false
}

// RFC 959 permits arbitrary intermediate lines; only the opening code followed
// by a space terminates a multiline reply.
func frameFTPReply(w []byte) (int, error) {
	n, err := frameCRLFLine(w, mailLineMax)
	if err != nil || n == 0 {
		return n, err
	}
	if n < 5 || !smtpReplyPrefix(w) && w[0] != '1' {
		return 0, fmt.Errorf("ftp: invalid reply code")
	}
	if n == 5 || w[3] == ' ' {
		return n, nil
	}
	if w[3] != '-' {
		return 0, fmt.Errorf("ftp: invalid reply separator")
	}
	for total := n; total < len(w); {
		next, err := frameCRLFLine(w[total:], mailLineMax)
		if err != nil {
			return 0, err
		}
		if next == 0 {
		...
		if end > mailReplyMax {
			return 0, fmt.Errorf("ftp: reply exceeds limit")
		}
		if next >= 6 && bytes.Equal(w[total:total+3], w[:3]) && w[total+3] == ' ' {
			return end, nil
		}
		total = end
	}
	if len(w) > mailReplyMax {
		return 0, fmt.Errorf("ftp: reply exceeds limit")
	}
	return 0, nil
}
