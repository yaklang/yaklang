	...
	for _, i := range publicPairs {
		...
		if dstIp := net.ParseIP(dstTarget); dstIp != nil {
			...
			go func() {
				...
				log.Debugf("create fin packet for %v", utils.HostPort(dstIp.String(), i.port))
				layers, loopback, err := s.createTCPWithDstMac(dstIp, i.port, nil)
				if err != nil {
					log.Warnf("cannot create fin-tcp packet for %s:%v err: %v", dstIp.String(), i.port, err)
					return
				}
				...
				if len(addrs) > 0 {
					for _, ip := range addrs {
						if dstIp := net.ParseIP(ip); dstIp != nil {
							layers, loopback, err := s.createTCPWithDstMac(dstIp, i.port, nil)
							if err != nil {
								log.Warnf("cannot create fin-tcp packet for %s:%v: %v", dstIp.String(), i.port, err)
								continue
							}
							s.inject(loopback, layers...)
						}
	...
	for targetIP, hwAddr := range results {
		...
		for _, port := range ports {
			...
			go func() {
				...
				layers, loopback, err := s.createTCPWithDstMac(dstIP, port, hwAddr)
				if err != nil {
					log.Warnf("cannot create fin-tcp packet for %s err: %s", utils.HostPort(dstIP.String(), port), err)
					return
				}
				...
				if count >= 5000 {
					count = 0
					log.Infof("fin scanner sent 5000 packets... total: %v", total)
				}
			}()
		}
	}
