...
func (hs *clientHandshakeStateGM) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		for i, asn1Data := range certMsg.certificates {
			...
			switch i {
			case 0:
				if (cert.KeyUsage == 0 || (cert.KeyUsage&(x509.KeyUsageDigitalSignature|cert.KeyUsage&x509.KeyUsageContentCommitment)) == 0) && !hs.allowServerCertsUsingWeakEncryption {
					c.sendAlert(alertInsufficientSecurity)
			...
			case 1:
				if (cert.KeyUsage == 0 || (cert.KeyUsage&(x509.KeyUsageDataEncipherment|x509.KeyUsageKeyEncipherment|x509.KeyUsageKeyAgreement)) == 0) && !hs.allowServerCertsUsingWeakEncryption {
					c.sendAlert(alertInsufficientSecurity)
