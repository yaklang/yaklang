...
// HTTP 客户端连接测试
func TestSimpleNewHTTPSClientToGMWebsite(t *testing.T) {
	/*
		HTTPS 连接测试
	*/
	time.Sleep(time.Second)
	httpClient := NewHTTPSClientSkipVerify()
	response, err := httpClient.Get("https://ebssec.boc.cn/boc15/login.html")
	if err != nil {
		t.Fatal(err)
	}
	defer response.Body.Close()
...
// HTTP 客户端连接测试
func TestSimpleNewHTTPSClientToGMWebsiteUseVanillaTransport(t *testing.T) {
	/*
		HTTPS 连接测试
	*/
	time.Sleep(time.Second)
...
// HTTP 客户端连接测试
func TestSimpleNewHTTPSClientToTraditionalWebsite(t *testing.T) {
	/*
		HTTPS 连接测试
	*/
	time.Sleep(time.Second)
	...
	response, err := httpClient.Get("https://baidu.com/")
	if err != nil {
		t.Fatal(err)
	}
	defer response.Body.Close()
