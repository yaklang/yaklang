...
func (e RecordHeaderError) Error() string { return "gmtls: " + e.Msg }
...
func (c *Conn) handshakeContext(ctx context.Context) (ret error) {
	...
	if c.handshakeErr == nil && !c.handshakeComplete() {
		panic("gmtls: handshake should have had a result.")
	}

	if c.handshakeErr != nil && c.handshakeComplete() {
		panic("gmtls: internal error: handshake returned an error but is marked successful")
	}
	return c.handshakeErr
}
...
func (c *Conn) VerifyHostname(host string) error {
	...
	defer c.handshakeMutex.Unlock()
	if !c.isClient {
		return errors.New("gmtls: VerifyHostname called on TLS server connection")
	}
	if !c.handshakeComplete() {
		return errors.New("gmtls: handshake has not yet been performed")
	}
	if len(c.verifiedChains) == 0 {
		return errors.New("gmtls: handshake did not verify certificate chain")
	}
