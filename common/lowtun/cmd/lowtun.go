	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				...
				stackInstance.IsSubnetBroadcast(1, ipv4.ProtocolNumber, tcpip.AddrFromSlice([]byte{255, 255, 255, 255}))
				...
				client := gvisorDHCP.NewClient(stackInstance, 1, 5*time.Second, 5*time.Second, 5*time.Second, func(ctx context.Context, lost, acquired tcpip.AddressWithPrefix, cfg gvisorDHCP.Config) {
					...
					err = netstack.WithMainNICIP(1, tcpip.AddrFromSlice(preferIp), macAddr)(stackInstance)
					if err != nil {
				...
				go func() {
					...
					// conn, err := gonet.DialContextTCP(utils.TimeoutContextSeconds(10), stackInstance, tcpip.FullAddress{
					// 	NIC:  1,
