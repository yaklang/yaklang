...
// when `+build darwin,go1.11` is added to the file, the following code will be included
...
func openDev(config *Config) (ifce *Interface, err error) {
	if config.Driver == Driver_MacOSDriverTunTapOSX {
		return openDevTunTapOSX(config)
	}
	if config.Driver == Driver_MacOSDriverSystem {
...
func openDevSystem(config *Config) (ifce *Interface, err error) {
	...
	// lower 1.11 in golang, check if the flag set
