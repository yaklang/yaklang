...
// GetPostgresParams returns connection params from config, with env fallback
func (config *PostgresConfig) GetPostgresParams() string {
	if config == nil {
...
func StartPostgres(pgdir string) error {
	return StartPostgresWithConfig(pgdir)
}
