...
func (function aiRuntimeRoundTripFunc) RoundTrip(request *http.Request) (*http.Response, error) {
...
func TestBuildYakAIEngineOptionsDoesNotInlineAttachmentWorkspaceFiles(t *testing.T) {
	...
	config := aiengine.NewAIEngineConfig(options...)
	if requested || len(config.AttachedResources) != 0 {
		t.Fatalf("attachment workspace leaked into prompt resources: requested=%v resources=%#v", requested, config.AttachedResources)
	}
}
