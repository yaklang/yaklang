...
func (c *RuleSyncClient) downloadSnapshotBundle(
	...
	result, err, _ := c.downloads.Do(normalizedID, func() (any, error) {
		...
		if validationErr := validateRuleSnapshotBundle(bundle, normalizedID); validationErr != nil {
			return nil, validationErr
		}
		bundle.Items = canonicalRuleSnapshotItems(bundle.Items)
		if cacheErr := c.cacheSnapshotBundle(bundle); cacheErr != nil {
...
func (c *RuleSyncClient) loadCachedSnapshotBundle(
	...
	if err := validateRuleSnapshotBundle(bundle, ""); err != nil {
		return nil, err
	}
...
func (c *RuleSyncClient) cacheSnapshotBundle(bundle RuleSnapshotBundle) error {
	c.mu.Lock()
	defer c.mu.Unlock()

	if err := validateRuleSnapshotBundle(bundle, bundle.SnapshotID); err != nil {
		return err
	}
	contentSHA := bundle.ContentSHA256
	canonical, err := json.Marshal(bundle)
	if err != nil {
		return utils.Wrap(err, "marshal canonical snapshot bundle")
	}
	...
	if !c.snapshotObjectReady(contentSHA) {
		...
		if err := writeRuleSnapshotFile(filepath.Join(tempDir, "bundle.json"), canonical, 0o600); err != nil {
			return err
		}
...
func (c *RuleSyncClient) snapshotObjectReady(contentSHA string) bool {
	...
	bundle, err := decodeRuleSnapshotBundle(raw)
	if err != nil || validateRuleSnapshotBundle(bundle, "") != nil {
		return false
	}
