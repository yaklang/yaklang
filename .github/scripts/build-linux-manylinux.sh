...
docker run --rm \
  ...
  bash -leo pipefail -c '
    ...
    ldflags="-s -w -X 'main.goVersion=${GO_VER_STR}' -X 'main.gitHash=${GIT_HASH}' -X 'main.buildTime=${BUILD_TIME}' -X 'main.yakVersion=${YAK_TAG}'"
    args+=(-ldflags "$ldflags" -o "./${OUTPUT_BINARY}" -v common/yak/cmd/yak.go)
    echo "Executing: ${args[*]}"
    "${args[@]}"
    ls -lh "./${OUTPUT_BINARY}"
  '
